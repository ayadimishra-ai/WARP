...
import { isUserAllowedAIFeature } from "@warp/client/features/form/common-functions";
import { ReviewerRemark, ReviewerStatusEntry } from "@warp/client/features/form/reviewer-context";
...
import { useBulkUpsertReviewerDetailsMappingMutation } from "@warp/graphql/mutations/generated/bulk-upsert-reviewer-details-mapping";
import { useInsertMakerCheckerRemarkMutation } from "@warp/graphql/mutations/generated/insert-maker-checker-remark";
import { useInsertReviewerDetailsMappingMutation } from "@warp/graphql/mutations/generated/insert-reviewer-details-mapping";
import { useUpdateFormInvitationStatusMutation } from "@warp/graphql/mutations/generated/update-form-invitation-status";
import { useUpdateReviewerDetailsMappingMutation } from "@warp/graphql/mutations/generated/update-reviewer-details-mapping";
import { useUpdateSubmissionStatusMutation } from "@warp/graphql/mutations/generated/update-submission-status";
  ...
  const handleDeclineSubmit = useCallback(async (questionId: string, remark: string, skipEmail: boolean = false) => {
    ...
    try {
      ...
      if (mappingId) {
        ...
        if (shouldSendEmail) {
          fetch("/api/reviewer-declined-email", {
            method: "POST",
  ...
  const handleBulkAccept = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.insert_ReviewerDetailsMapping) {
        ...
        await fetch("/api/reviewer-aaproved-email", {
          method: "POST",
          body: JSON.stringify({
            id: invitationId,
        ...
        try {
          ...
          if (hasUserAICapabilities && hasAnyAICapabilities) {
            const questionaryName = `${
              formInvitation?.FormInvitation[0]?.Form?.name ||
              "Report"
            }`;
  ...
  return (
    <Stack className="assessmentDetails-Ques-Ans">
      ...
      <Modal
        ...
        title={<Text weight={600} size="lg">Reason for Decline</Text>}
        centered
        size="md"
      >
        <Stack spacing="md">
