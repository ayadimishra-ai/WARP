...
//import { DashboardWrapper, ShowAnalytic } from '@yagnitechdev/analytic';
...
class PredealESGReport extends Component {
    ...
    async getThemeScoredata() {
        ...
        if (JSON.parse(localStorage.userType) === RoleCodes.SUPPLIER || JSON.parse(localStorage.userType) === RoleCodes.BUYER || ShowOtherCompanyDetails === true) {
            IsVCUser = false;
            await axios.get(cPanelUrl + "/v1/assessments/portfolio_kpi_data?measure=predeal", config).then(async (json) => {
            ...
            IsVCUser = true;
            await axios.get(cPanelUrl + "/v1/assessments/esg_kpi_data?measure=predeal", config).then(async (json) => {
    ...
    downloadExcel = async (val) => {
        ...
        await axios
            .get(cPanelUrl + "/v1/assessments/esg_kpi_data_spreadsheet?for_themes=" + val, config)
            .then((json) => {
                // console.log(json.data.file_path)
                const link = document.createElement('a');
                link.href = cPanelUrl + '/' + json.data.file_path;
                link.setAttribute(
                    'download',
                    `FileName.pdf`,
                );
    ...
    render() {
        ...
        } else if (getUserPermision(permissions, PageKeys.PredealESGReport) === null) {
            return <Redirect to="/not-found" />;
        }
        ...
        { 'pageName': 'Supplier ESG Report', 'url': '/#' },
        ])
                    ...
                    {this.state.loader ? <Spinner /> : this.state.preDealData == null ?
                        ...
                        <div className=" esg_report_container">
                            <div className="esg_report_header">
                                ...
                                <h6>Supply Chain ESG Dashboard   </h6>
                            ...
                            {JSON.parse(localStorage.userType) === RoleCodes.SUPPLIER || JSON.parse(localStorage.userType) === RoleCodes.BUYER || ShowOtherCompanyDetails === true ?
                                <GridContainer className="top_esg_report">
                                    ...
                                    {this.state.preDealData.ESGMaturity.PortfolioList.length > 0 ?
                                                                                ...
                                                                                <td>
                                                                                    ...
                                                                                    <ul className="capsule_tab" style={{ alignItems: 'left' }}>
                                                                                        {data2.ThemeNames.map((data3, i) => {
                                                                                            if (data3 !== "") {
                                                                                                return (
                                                                                                    <li key={i} onClick={(e) => this.handlePopoverOpen(e, data3, data3, 'RiskRating')}>{data3}</li>
                                                                                                )
                                                                                            }
                                                                                        })}
                                                                                    </ul>
                                        ...
                                        : ""}
                                    {/* {this.state.preDealData.RecommendationList.length > 0 ?
                                        <GridItem md={12}>
                                            <div className="preDeal_esg portfolio_Predeal">
                                                    ...
                                                    <table className="preDeal_tables">
                                                        <thead>
                                                            <tr>
                                                                <th>Themes</th>
                                                                <th>Areas of work</th>
                                                                <th style={{ textAlign: 'right', paddingRight: '30px' }}>Term</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {this.state.preDealData.RecommendationList.map((data, i) => {
                                                                return (
                                                                    <tr>
                                                                        <td>{data.ThemeName}</td>
                                                                        <td className="riskRating">
                                                                            <ul style={{ alignItems: 'left' }}>
                                                                                {data.Recommendations.map((data3, i) => {
                                                                                    return (
                                                                                        <li>{data3}</li>
                                                                                    )
                                                                                })}
                                                                        ...
                                                                        <td style={{ textAlign: 'right', paddingRight: '20px' }}><span className={data.term}>{data.term}</span></td>
                                                                    </tr>
                                                                )
                                                            })}
                                                        </tbody>
                                    ...
                                    <GridContainer className="top_esg_report">
                                        <GridItem md={6}>
                                            <div className="preDeal_esg first_class">
                                                {/* <ShowAnalytic apiUrl={cPanelUrl} id={"overall_fund_performance"} style={{ height: '400px', width: '100%', border: 'none', padding: 0, display: "inline-flex" }}></ShowAnalytic> */}
                                            </div>
                                        </GridItem>
                                        {this.state.FemaleEmployeesdata !== null ?
                                            <GridItem md={6}>
                                                <div className="preDeal_esg">
                                                    <HighchartsReact highcharts={Highcharts} options={this.state.FemaleEmployeesdata} />
                                                </div>
                                            </GridItem>
                                            : ""}
                                        {this.state.EmployeeAttrition !== null ?
                                            <GridItem md={6}>
                                                <div className="preDeal_esg">
                                                    <HighchartsReact highcharts={Highcharts} options={this.state.EmployeeAttrition} />
                                                </div>
                                            </GridItem>
                                            : ""}
                                        {this.state.WomenInLeadershipPosition !== null ?
                                            <GridItem md={6}>
                                                <div className="preDeal_esg">
                                                    <HighchartsReact highcharts={Highcharts} options={this.state.WomenInLeadershipPosition} />
                                                </div>
                                            </GridItem>
                                            : ""}
                                        ...
                                        <GridItem md={12}>
                                            <div className="preDeal_esg">
                                                {/* <ShowAnalytic apiUrl={cPanelUrl} id={"esg_ategory_Score"} style={{ height: '300px', width: '100%', border: 'none', padding: 0, display: "inline-flex" }}></ShowAnalytic> */}
                                            </div>
                                        </GridItem>
                                        <GridItem md={12}>
                                            <div className="preDeal_esg">
                                                {/* <ShowAnalytic apiUrl={cPanelUrl} id={"themewise_score"} style={{ height: '300px', width: '100%', border: 'none', padding: 0, display: "inline-flex" }}></ShowAnalytic> */}
                                            </div>
                                        </GridItem>
                                        {/*4215 <GridItem md={12}>
                                            <div className="preDeal_esg">
                                        ...
                                        {this.state.preDealData.ThemeWiseRiskRating.ThemeList.length > 0 ?
                                            <GridItem md={12}>
                                                <div className="preDeal_esg">
                                                    <h5>Themewise Risk Rating Of The Companies (Based on response and process materiality)
                                                        <Button onClick={() => this.downloadExcel('All')}>
                                       ...
                                       {/* {this.state.preDealData.ThemeWiseRiskRating.ThemeList.length > 0 ?
                                            <GridItem md={12}>
                                                <div className="preDeal_esg">
                                                    <h5>Themewise Risk Rating Of The Companies (Based on response and process materiality) CP
                                                        <Button onClick={() => this.downloadExcel('CP')}>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0z" fill="none" /><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z" /></svg>
                                                        </Button>
                                                    </h5>
                                                    <div style={{ overflowX: 'auto' }}>
                                                        <table className="preDeal_tables">
                                                            ...
                                                            <tbody>
                                                                {this.state.preDealData.ThemeWiseRiskRating.ThemeList.filter((v) => v.CPCSValue == 'CP').map((data, i) => {
                                                                    return (
                                                                        <tr><td><span style={{ whiteSpace: 'nowrap' }}>{data.ThemeName}</span></td>{
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                        ...
                                        {this.state.preDealData.ThemeWiseRiskRating.ThemeList.length > 0 ?
                                            <GridItem md={12}>
                                                <div className="preDeal_esg">
                                                    <h5>Themewise Risk Rating Of The Companies (Based on response and process materiality) CS
                                                        <Button onClick={() => this.downloadExcel('CS')}>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0z" fill="none" /><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z" /></svg>
                                                        </Button>
                                                    </h5>
                                                    <div style={{ overflowX: 'auto' }}>
                                                        <table className="preDeal_tables">
                                                            ...
                                                            <tbody>
                                                                {this.state.preDealData.ThemeWiseRiskRating.ThemeList.filter((v) => v.CPCSValue == 'CS').map((data, i) => {
                                                                    return (
                                                                        <tr><td><span style={{ whiteSpace: 'nowrap' }}>{data.ThemeName}</span></td>{
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                            ...
                                            : ""}*/}
                                        {this.state.preDealData.PerformanceOnKPIs.Parameters.length > 0 ?
                                            <GridItem md={12}>
                                                <div className="preDeal_esg">
                                                    <h5>Performance On KPIs</h5>
                                                    <div>
                                                        <table className="preDeal_tables">
                                                            <thead>
                                                                <tr>
                                                                    <th>Parameters</th>
                                                                    {this.state.preDealData.PerformanceOnKPIs.Parameters.slice(0, 1).map((data, i) => {
                                                                        return (
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                                                                    <th>{this.toCamelCase(data2.CompanyName)}</th>
                                                            ...
                                                            <tbody>
                                                                {this.state.preDealData.PerformanceOnKPIs.Parameters.map((data, i) => {
                                                                    return (
                                                                        <tr><td>{data.Parameter}</td>{
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                                                                    <td>
                                                                                        <span className={this.toCamelCase(data2.CompanyName)}>
                                                                                            {data2.Score}
                                                                                        </span>
                                                                                    </td>
                                                                                )
                                        ...
                                        {this.state.preDealData.KeyDataPoints.Parameters.length > 0 ?
                                                    ...
                                                    <div>
                                                        <table className="preDeal_tables">
                                                            <thead>
                                                                <tr>
                                                                    <th>Parameters</th>
                                                                    {this.state.preDealData.KeyDataPoints.Parameters.slice(0, 1).map((data, i) => {
                                                                        return (
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                                                                    <th>{this.toCamelCase(data2.CompanyName)}</th>
                                                            ...
                                                            <tbody>
                                                                {this.state.preDealData.KeyDataPoints.Parameters.map((data, i) => {
                                                                    return (
                                                                        <tr><td>{data.Parameter}</td>{
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                            ...
                                            : ""}
                                        {this.state.preDealData.PolicySummary.Parameters.length > 0 ?
                                            <GridItem md={12}>
                                                <div className="preDeal_esg">
                                                    ...
                                                    <div>
                                                        <table className="preDeal_tables">
                                                            <thead>
                                                                <tr>
                                                                    <th>Parameters</th>
                                                                    {this.state.preDealData.PolicySummary.Parameters.slice(0, 1).map((data, i) => {
                                                                        return (
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                                                                    <th>{this.toCamelCase(data2.CompanyName)}</th>
                                                            ...
                                                            <tbody>
                                                                {this.state.preDealData.PolicySummary.Parameters.map((data, i) => {
                                                                    return (
                                                                        <tr><td>{data.Parameter}</td>{
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                        ...
                                        <GridItem md={12}>
                                            <div className="preDeal_esg">
                                                <h5>Current Practices</h5>
                                                <div className="current_practices">
                                                    <table className="preDeal_tables">
                                                        <thead>
                                                            <tr>
                                                                <th>Parameters</th>
                                                                {this.state.preDealData.CurrentPractices.Parameters.slice(0, 1).map((data, i) => {
                                                                    return (
                                                                        data.CompanyList.map((data2, i) => {
                                                                            return (
                                                                                <th>
                                                                                    {this.toCamelCase(data2.CompanyName)}
                                                                                </th>
                                                                            )
                                                                        })
                                                                    )
                                                        ...
                                                        <tbody>
                                                            {this.state.preDealData.CurrentPractices.Parameters.map((data, i) => {
                                                                return (
                                                                    <tr className={data.Parameter}>
                                                                        <td>
                                                                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                                                                <img alt={data.Parameter} src={data.Parameter === 'Governance' && predealGovernance || data.Parameter === 'Social' && predealSocial || data.Parameter === 'Environment' && predealEnvironment} />
                                                                                <p style={{ color: data.Parameter === 'Governance' && '#012169' || data.Parameter === 'Social' && '#FF9E1B' || data.Parameter === 'Environment' && '#008522', margin: '0 0 0 10px' }}>{data.Parameter}</p>
                                                                            </div>
                                                                        </td>
                                                                        {
                                                                            data.CompanyList.map((data2, i) => {
                                                                                return (
                                                                                    <td key={i}>
                                                                                        {data2.Value.map((data3, i) => {
                                                                                            return (
                                                                                                <span style={{ color: data3.Tag === 'Red' ? '#da7d00' : '#1a1a1a' }} key={i} className={data3 !== null ? data3.Value.toLowerCase() : ""}>
                                                                                                    {data3.Value}
                                                                                                </span>
