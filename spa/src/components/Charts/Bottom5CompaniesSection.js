...
class Bottom5CompaniesSection extends Component {
  constructor(props) {
    super(props);
    this.state = {
      anchorEl: null,
  ...
  componentDidMount() {
    ...
    //   query: "query MyQuery{gettotalsectoresg(args:{parentcompanyid:\"ce6c5fee-10ef-4cf7-af5b-1a6d6d83645b\"},order_by:{sector:asc,score:asc}){company_name sector score}}",
    //   variables: {}
    // });
    ...
    //let sorted_data = orderBy(chartData, ['sector', 'score'], ['asc', 'asc']);
    this.setState({ List: chartData }, () => {
      this.aggregateData();
    });
  }
  selectChangeESG = (event, rowNumber) => {
    if (event.target.value !== "All") {
      let getSelectESGData = this.props.TotalSectorESGData.filter(
        (x) => x.esg_section_name === event.target.value
      );
      this.setState(
        { List: getSelectESGData, selectedEsg: event.target.value },
        () => {
          this.aggregateData();
        }
      );
      ...
      this.setState(
        {
          ...
          selectedEsg: event.target.value,
        },
        () => {
          this.aggregateData();
        }
  ...
  aggregateData() {
    ...
    // Sorting the aggregated data by score in ascending order
    aggregatedData.sort((a, b) => a.score - b.score);
  ...
  render() {
    ...
    // Display the bottom 5 aggregated companies
    const bottom5AggregatedCompanies = aggregatedData.slice(0, 5);

    return (
      <div className="chart_table">
        <ChatHeadingSubHeading title="Bottom 5 Companies by Section">
          <div>
            <span>ESG: </span>
            <Select
              value={this.state.selectedEsg}
              onChange={(event) => this.selectChangeESG(event)}
              displayEmpty
              name="esg"
              className={"header_select"}
            >
              {this.state.esgSelectData.map((option) => (
        ...
        <table>
          ...
          <tbody>
            {bottom5AggregatedCompanies.length === 0 ? (
              <tr>
              ...
              bottom5AggregatedCompanies.map((item, index) => (
                <tr key={index}>
...
export default Bottom5CompaniesSection;
