...
import { getPageResource, getBuyerPreferences, getElasticData } from '../../utility';
import { getLanguageResourceElasticIndex, getWebsiteLanguageGuid, getServiceUrl, getElasticIndexNew } from '../../config';
...
class Wishlist extends Component {
    ...
    async componentDidMount() {
        ...
        if (this.props.wishlistCounter === 0) {
    ...
    getWishListData = async() => {
        await getBasketDetails(this.props.userId, localStorage.companyGuid, localStorage.languageId)
            .then(json => {
                if (json.data !== null) {
            ...
            await fetchWishListData(this.props.userId, localStorage.companyGuid, localStorage.languageId, 5, 0, null)
            .then(json => {
                let productGuids = [];
                if (json.data !== null) {
                    
                    // this.setState({
                    //     wishListDetails: json.data.table1, loading: false
                    // })
                    wishlistDetails = json.data.table1
                    productGuids = [...new Set(json.data.table1.map(x => x.productGuid))]
                }
                this.setState({
                    //wishListData: wishlistDetails, loading: false,
                    wishListProductData: wishlistDetails,
                    basketData: basketDetails, wishlistProductList: productGuids
                })
    ...
    wishListViewAll = (event) => {
        this.context.router.history.push('/wishlist');
        // this.handleClose_WishL();
    ...
    getBuyerPreferences = async () => {
        await getBuyerPreferences(localStorage.userId, localStorage.companyGuid)
            .then((json) => {
                ...
                this.getIndexData(buyerBusinessTypeForIndex, buyerProductLevelCertificatesForIndex, buyerSupplierLevelAdditionalCertificatesForIndex, buyerSupplierLevelMandatoryCertificatesForIndex, tildeSepratedBuyerProductCategories);
            }).catch(err => err.response !== undefined ? err.response.status === 401 ? window.location.pathname = '/' : '' : '');
    }

    getIndexData = (buyerBusinessTypeForIndex, buyerProductLevelCertificatesForIndex, buyerSupplierLevelAdditionalCertificatesForIndex, buyerSupplierLevelMandatoryCertificatesForIndex, tildeSepratedBuyerProductCategories) => {
        let indexName = "";
            ...
            query: {
                bool: {
                    must: [
                        ...
                        { terms: { "productguid_raw.raw.keyword": this.state.wishlistProductList } },
            ...
            <React.Fragment>
                <div style={({ display: this.state.loading ? 'none' : 'block' })}>
                    <div className="wishlist_dropdown">
                        <h5>Wishlist </h5>
                        <h3>Recently Added <span>({this.state.wishListData !== undefined ? this.state.wishListData.length : ''})</span>  </h3>
                        <Tooltip title="View All"><span className="redirect_wishlist" onClick={(event) => this.wishListViewAll(event)}><Launch /></span></Tooltip>
                        {!this.state.noWishlist && this.state.wishListData.length !== 0 ? <ul>
                            {this.state.wishListData !== undefined ?
                        ...
                        <div className="no_notification">
                           ...
                           <p>Currently there are no products in your Wishlist.</p>
                       </div>: ""}
                    </div>
                </div>
