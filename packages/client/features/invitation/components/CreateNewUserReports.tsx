...
import { useGetFormSubmissionByInvitationsLazyQuery } from "@warp/graphql/queries/generated/get-form-submission-by-invitation-id";
      ...
      <Stack spacing="md" px={15}>
              ...
              <Stack spacing={0}>
                <Controller<FormValues>
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <MultiSelect
                        data={filterUserNameList.map((item) => ({
                          value: item.id,
                          label: item.name,
                        }))}
                        styles={(theme) => ({
                        ...
                        onCreate={(query) => {
                          // Validate that the name only contains alphabetic characters and spaces
                          const nameRegex = /^[A-Za-z\s]+$/;
                          if (!nameRegex.test(query)) {
                            // If name doesn't match the pattern, show an error
                            setTimeout(() => {
                          ...
                          setFilterUserNameList((prev) => [...prev, newItem]);
                          ...
                          // Return the new item for MultiSelect
                        ...
                        onChange={(value) => {
                          ...
                          // Clear validation errors when the field changes
                          clearErrors(`userForm.${index}.fullName`);

                          // Clear everything if no value is selected
                          ...
                          if (selectedUser) {
                            ...
                            if (!nameRegex.test(selectedUser.name)) {
                              ...
                              // Clear email validation errors as well
                              clearErrors(`userForm.${index}.fullName`);
                              clearErrors(`userForm.${index}.email`);
                            }
                          } else {
        ...
        <>
          {(formIdError && (
            ...
            >
              Select Reporting Framework is required
            </Text>
          )) ||
