...
export async function fetchDocuments(
    ...
    // Import GraphQL operations dynamically to avoid circular dependencies
    const { GetDocumentLogsDocument: GET_DOCUMENT_LOGS } = await import("@warp/graphql/queries/generated/get-document-logs");
...
export async function fetchDocumentsWithFiltering(
    ...
    const { mergeWithLatestSystemGenerated } = await import("../domain/document.selectors");

    // Step 1: Fetch raw documents from database
    const rawDocuments = await fetchDocuments(apolloClient, userContext);
...
export async function fetchFormInvitationsMapping(
    ...
    // Import GraphQL query dynamically
    const { GetFormInvitationDetailsbyIdDocument } = await import(
        "@warp/graphql/queries/generated/get-form-invitation-details-by-id"
    );
    ...
    await Promise.all(
        invitationIds.map(async (invitationId) => {
            try {
                const { data, error } = await apolloClient.query({
                    query: GetFormInvitationDetailsbyIdDocument,
                    variables: {
...
export async function fetchDocumentTemplates(
    ...
    try {
        const { GetAiSuggestedDocumentsDocument: GET_AI_SUGGESTED_DOCUMENTS } = await import("@warp/graphql/queries/generated/get-AI-suggested-documents");

        const { data, error } = await apolloClient.query({
            query: GET_AI_SUGGESTED_DOCUMENTS,
            fetchPolicy: "network-only", // Changed to network-only for SSR
        });
...
export async function fetchSubscriptionStatus(
    ...
    const { GetActiveSubscriptionByCompanyIdDocument: GET_ACTIVE_SUBSCRIPTION_BY_COMPANY_ID } = await import("@warp/graphql/queries/generated/get-active-subscription-by-company-id");

    const { data, error } = await apolloClient.query({
        query: GET_ACTIVE_SUBSCRIPTION_BY_COMPANY_ID,
        variables: {
    ...
    if (error) {
        ...
        return { isChatSubscriptionActive: false };
    }
...
function normalizeTemplateFromGraphQL(raw: any): DocumentTemplate {
    ...
    if (raw.acceptedFormats) {
        ...
        else if (typeof raw.acceptedFormats === 'object' && raw.acceptedFormats.acceptedFormats) {
        ...
        // If it's a JSON string, parse it and extract the nested array
            ...
            try {
                ...
                console.warn('[normalizeTemplateFromGraphQL] Failed to parse acceptedFormats:', raw.acceptedFormats, error);
                acceptedFormats = [];
            }
        }
    }
