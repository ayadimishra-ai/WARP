...
import { useUpdateFormSubmissionStatusMutation } from "@warp/graphql/mutations/generated/update-form-submission-status";
import { useUpdateReviewerDetailsMappingMutation } from "@warp/graphql/mutations/generated/update-reviewer-details-mapping";
import { useGetDeclinedQuestionsCountLazyQuery } from "@warp/graphql/queries/generated/get-declined-questions-count";
...
export const useReviewerActions = (invitationId: string) => {
    ...
    const onReviewerDecline = useCallback(async (questionId: string, remark: string) => {
        ...
        try {
            ...
            if (mappingId) {
                ...
                if (shouldSendEmail) {
                    try {
                        await fetch("/api/reviewer-declined-email", {
                            method: "POST",
    ...
    const onReviewerJustify = useCallback(async (questionId: string, remark: string) => {
        ...
        try {
            ...
            if (shouldSendEmail) {
                try {
                    await fetch("/api/reviewer-resubmit-email", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                            id: invitationId,
