...
>(
  ...
  input: any
): Omit<typeof _rule, keyof RuleType> | null => {
  ...
  try {
    ...
    const result = Boolean(expression.evaluate(input)) ? rest : null;
    return result;
  } catch (error) {
...
export const selectFieldOptions = <T extends FormFieldInterfaceKeysType>(
  ...
  if (JSON.stringify(calculatedOptions) === JSON.stringify(initialFieldOptions)) {
    return initialFieldOptions;
  }
...
export const selectInterfaceOptions = <T extends FormFieldInterfaceKeysType>(
  ...
  if (JSON.stringify(calculatedOptions) === JSON.stringify(initialInterfaceOptions)) {
    return initialInterfaceOptions;
  }
...
export const selectDisplayOptions = <T extends FormFieldInterfaceKeysType>(
  ...
  if (JSON.stringify(calculatedOptions) === JSON.stringify(initialDisplayOptions)) {
    return initialDisplayOptions;
  }
...
export const useChildFieldState = <T extends FormFieldInterfaceKeysType>(
  ...
  const fieldState = useFormFieldStore(
    ...
    (prev, next) => {
      ...
      return (
        prev.value === next.value &&
        JSON.stringify(prev.fieldOptions) === JSON.stringify(next.fieldOptions) &&
        JSON.stringify(prev.interfaceOptions) === JSON.stringify(next.interfaceOptions) &&
        JSON.stringify(prev.displayOptions) === JSON.stringify(next.displayOptions)
      );
    }
  );
...
export const selectSimpleFieldOptions = <T extends FormFieldInterfaceKeysType>(
  ...
  let context = store.answer;
  if (ansId && formField.groupField) {
    const parentField = store.formFields.find(
      (f) => f.field === formField.groupField
    );
    if (parentField) {
      const parentAnswer = store.answer[parentField.field];
      if (parentAnswer && Array.isArray(parentAnswer.value)) {
        const rowData = parentAnswer.value.find((a: any) => a._id === ansId);
        if (rowData) {
          context = { ...context, ...rowData };
        }
      }
    }
  }
