...
import { useUpsertFormInvitationCompletionMutation } from "@warp/graphql/mutations/generated/upsert-form-invitation-completion-by-invitation-id";
...
import { useUpdateAssesseeUserMappingResponderStatusByResponderMutation } from "@warp/graphql/mutations/generated/update-assesseeusermapping-responderstatus-byresponder";
...
import { isFormAIEnabled } from "@warp/shared/utils/jwt-ai.util";
...
const GroupTab: FC<GroupTabsPropsType> = ({
  ...
  // console.log("reviewerStatusMap", reviewerStatusMap);
  ...
  useEffect(() => {
    ...
    const findQuestions = (field: any) => {
      if (field?.Question?.id) {
        const hasAnswer = isAnswered(field, false, chkStore);
  ...
  }, [steps, chkStore.answer, updateQuestionStatuses]);
  ...
  const getProgressBarPercentageForListing = useCallback(() => {
    ...
    const findQuestions = (field: any) => {
      if (field?.Question?.id) {
        // Use the imported isAnswered function to check status in real-time
        statusMap[field.Question.id] = isAnswered(field, false, chkStore);
        return;
      }
    ...
    // Calculate count of answered required questions
  ...
  }, [steps, chkStore, requiredQuestions]);
  ...
  const submitConfirmed = async () => {
    ...
    if (isResponderUsercheck === "true") {
      ...
      if (formInvitation?.reviewerDetails) {
        ...
        await fetch("/api/calculate-score/reviewer-form-submission-email", {
          method: "POST",
          body: JSON.stringify({
            formId,
            submissionId,
        ...
        try {
          ...
          if (allowReportGeneration) {
            ...
            const response = await fetch("/api/AI/generate-background-report", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: userSession?.accessToken ?? "",
  ...
  const getProgressBarPercentage = () => {
    // Calculate progress for current section in real-time
    if (!steps || !steps.length || !currentSec) return "0";

    const currentStep = steps.find((s: any) => s.key === currentSec);
    if (!currentStep) return "0";

    const field = currentStep.component?.props?.formField;
    if (!field) return "0";

    let total = 0;
    let answered = 0;

    const findQuestions = (f: any) => {
      if (f?.Question?.id) {
        if (!hasAnyInputFields(f)) return; // Don't count purely informational questions in progress
        total++;
        if (isAnswered(f, false, chkStore) || !isMandatoryQuestion(f)) {
          answered++;
        }
        return;
      }
      if (f?.children && Array.isArray(f.children)) {
        f.children.forEach(findQuestions);
      }
    };
  ...
  const saveBtn = async (e: any) => {
    ...
    if (query?.mode === FormMode.Start) {
      ...
      fetch("/api/reviewer-resubmit-email", {
        method: "POST",
  ...
  return (
    <Flex
      ...
      align="flex-start"
    >
      <Stack ref={elementRef} sx={{flex: 1, minWidth: 0}}>
        {/* <LoadingOverlay
            ...
            <Stack spacing={10}>
              {statusEntry.remarks.filter((r) => r.remark !== "Answered by Maker").map((r, idx) => (
                <Box key={idx} >
                  ...
                  <Text size="xs" ta="right" color="#666666">
                    Date & Time:{" "}
