      ...
      const handleBCMessage = (ev: MessageEvent) => {
        ...
        if (payload?.type === "add-files-directly-to-answer") {
          ...
          if (payload.data?.formFieldId === formField.id) {
            ...
            const currentAnswerFiles = Array.isArray(state.value)
              ? state.value
              : [];

            const updatedAnswerFiles = formField.interfaceOptions?.allowMultiple
              ? [...currentAnswerFiles, ...newAnswerFiles]
              : [...newAnswerFiles];

            state.setValue(formField.id, updatedAnswerFiles);
            clearFields();
          }
