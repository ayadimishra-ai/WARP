...
const useStyles = createStyles((theme) => ({
  table: {
    ...
    "& .has-suggestions":{
      "& input": {
        paddingRight: "30px", // Add padding to prevent text overlap with AI icon
        marginRight: "-30px",
      },
    ...
    "& .uploaded-doc-td": {
      ...
      "& .classNameforFixedTable:has(.mantine-Input-suffix)": {
        "& input": {
          paddingLeft: "30px",
        },
...
const FixedQuestionTable: FormFieldControl<"fixed-question-table"> = ({
    ...
    formField.children.forEach((childField: any) => {
      ...
      if (
        ...
        try {
          ...
          let calculatedValue = expression.evaluate(allData);
  ...
  // Include ALL fields - regular, header, and footer fields
  const fieldsByRow: Record<string, any[]> = {};

  formField?.children?.forEach((childField: any) => {
    const rowId = childField?.interfaceOptions?.rowId || "default";
    if (!fieldsByRow[rowId]) {
      fieldsByRow[rowId] = [];
    }
    fieldsByRow[rowId].push(childField);
  });

  // Sort fields within each row by columnId, with fallback to field name number
  Object.keys(fieldsByRow).forEach((rowId) => {
    fieldsByRow[rowId].sort((a, b) => {
      // Primary sort: by columnId if both have it
      ...
      // Fallback: sort by the numeric suffix in field name (e.g., s3_5_1_q1_1_12 -> 12)
                    ...
                    !fieldWithoutLabel.fieldOptions?.readonly && (
                      ...
                      >
                        <ActionIcon
                          ...
                          pos="absolute"
                          top={5}
                          right={childFormField.interface === "input-multiline" ? 10 : 4}
                          sx={{
                            zIndex: 10,
                            cursor: "pointer",
                          }}
                          bg="#fff"
                        >
                          {!loading && (isAIUser ? <SparkleGradientIcon /> : <CarryForwardAnswerIcon />)}
                        </ActionIcon>
                      </Tooltip>
                    )}
                </Flex>
          ...
          carouselField && (
            ...
            >
              <Flex align="center" gap={10} mb={15}>
                {!loading && isAIUser && <SparkleGradientIcon />}
