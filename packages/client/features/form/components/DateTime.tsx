...
const DateTimeInputCommon = ({
  ...
  return (
    <Stack
      w="100%"
      onClick={(e) => {
        // Prevent onClick from firing when DatePicker is being interacted with
        if ((e.target as HTMLElement).closest('.mantine-DatePicker-root, .mantine-DateRangePicker-root, .mantine-DatePicker-dropdown, [role="dialog"]')) {
          e.stopPropagation();
          return;
        }
      ...
      spacing="sm"
    >
      <DisplayLabel text={label} infoIconProps={infoIconProps} formField={formField} />
      {isRange ? (
        <DateRangePicker
          withinPortal
...
const SimpleDateTimeInput: FormFieldControl<"datetime"> = ({
  ...
  if (!name) return <></>;
  return (
    <Stack spacing="sm" style={pointerEventsStyle}>
