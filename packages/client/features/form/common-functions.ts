...
const checkValidation = (
  ...
  try {
    ...
    if (debugContext) {
      ...
      if (debugContext.formFieldId) console.log(`  - FormField ID: ${debugContext.formFieldId}`);
      if (debugContext.formFieldName) console.log(`  - FormField Name: ${debugContext.formFieldName}`);
      if (debugContext.questionId) console.log(`  - Question ID: ${debugContext.questionId}`);
      if (debugContext.questionTitle) console.log(`  - Question Title: ${debugContext.questionTitle}`);
      if (debugContext.interfaceType) console.log(`  - Interface Type: ${debugContext.interfaceType}`);
      if (debugContext.ruleIndex !== undefined) console.log(`  - Validation Rule Index: ${debugContext.ruleIndex}`);
    }
    console.groupEnd();
...
export const isAnswered = (
  ...
  if (!!children?.length) {
    const childrenResults: {
      ...
      answered: isAnswered(child, false, chkStore),
    ...
    if (isParentInputType) {
      ...
      localIsAnswered = allRequiredAnswered;
    }
  }
...
export const AIFeatureForInvitationId = (
  ...
  const aiData = getFormAIPlans(userSession?.accessToken, formId);
  const transformedAIData =
    aiData.length > 0
      ? [
        {
          formId: formId,
          docWithAI: hasFullAICuration(userSession?.accessToken, formId), // Both DocumentCuration + WebCuration
          onlyDoc:
            hasDocumentCuration(userSession?.accessToken, formId) &&
            !hasWebCuration(userSession?.accessToken, formId), // Only DocumentCuration
...
export const getAIProcesingStats = async (
  ...
  if (!!formId && !!invitationId) {
    ...
    await fetch(
      apiBase + "/api/AI/AI-dataStats-calculation",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
...
export const callAIAPI = async (formInvitationData: webCurationAPIDataType) => {
  /**
   * Root cause: NEXT_PUBLIC_ env vars in Next.js are inlined at build time, not runtime.
   * If NEXT_PUBLIC_API_BASE_URL was unavailable during build, it becomes undefined in the
   * browser bundle. JavaScript string concatenation then produces:
   * "undefined" + "/api/AI/AIprocessing" = "undefined/api/AI/AIprocessing" (a relative string, not absolute).
   * The browser resolves this relative to the current page path (/embed/form/invitation/.../),
   * resulting in the broken URL: .../undefined/api/AI/AIprocessing.
   *
   * Fix: On the client, use "" as the base so /api/AI/... always resolves from the site root,
   * completely bypassing the missing env var problem. On the server, keep NEXT_PUBLIC_API_BASE_URL
   * since server-side fetch requires an absolute URL and the variable is reliably available at runtime.
   */
  const apiBase =
    typeof window !== "undefined"
      ? ""
      : (process.env.NEXT_PUBLIC_API_BASE_URL ?? "");
  await fetch(apiBase + "/api/AI/AIprocessing", {
    method: "POST",
    headers: {
      "content-type": "application/json",
    },
...
export const callUploadDocumentIngestingAPI = async (
  ...
  const apiBase =
    typeof window !== "undefined"
      ? ""
      : (process.env.NEXT_PUBLIC_API_BASE_URL ?? "");
  const response = await fetch(apiBase + "/api/AI/AIprocessing", {
    method: "POST",
    headers: {
      "content-type": "application/json",
    },
