...
export const carryForwardAssessmentDataUserWise = async (body: any) => {
  ...
  for (
    let lastinviteData = 0;
    lastinviteData < body.lastInvitationDataQuery.length;
    lastinviteData++
  ) {
    let lastInvitationDataQuery: any = [];
    if (
      body.lastInvitationDataQuery[lastinviteData].Company?.LastFormInvitation
        ?.length > 0
    ) {
      lastInvitationDataQuery =
        body.lastInvitationDataQuery[lastinviteData].Company
          ?.LastFormInvitation;
    } else {
      lastInvitationDataQuery =
        body.lastInvitationDataQuery[lastinviteData].Company
          ?.LastFormInvitationWithoutUserId;
    }
    if (
      ...
      if (insertFormInvitationResult && insertFormInvitationResult.length > 0) {
        ...
        if (
          ...
          if (!!invitationIdArray && invitationIdArray.length > 0) {
            const updateStatus =
              await sdk.updateInvitationToDraftByInvitationId({
                ...
                lastInvitationDataQuery
                  ...
                  ?.Interim_Answers?.map((item: any) => {
                    ...
                    if (item?.Interim_Answer != null) {
                      if (
                        item?.Interim_Answer?.Interim_Recommendations?.length >
                        0
                      ) {
                        submissionIdrelations.push({
            ...
            const newRaraValidationRating: any =
              await sdk.getvalidationandrating({
                invitationId: oldinvitationid_array,
              });
              ...
              if (updatedata.length > 0) {
                const updateinterimAnswer =
                  await sdk.updateAnswerIdinInterimAnswer({
                    InterimAnswerUpdate:
                      updateinterimanswer.InterimAnswerUpdate,
