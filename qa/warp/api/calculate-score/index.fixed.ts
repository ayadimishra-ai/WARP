...
// FIX: Removed module-level `let recommendationWithFormfieldData: any = []`
// The original was shared across all concurrent requests — a critical race condition
// that mixed recommendation data between users. It is now a local variable inside
// calculateScore() and passed explicitly to fillrecommendationArray().
...
const fillrecommendationArray = (
  ...
  invitedBy: any,
  isAnswer: boolean,
  recommendationWithFormfieldData: any[] // FIX: explicit parameter, not module-level closure
) => {
...
const calculateScore = async (
  ...
  // FIX: Local variable — fresh array per request, no cross-request contamination
          ...
          if (
            ...
            if (
              ...
              if (!!allCheckBoxes && allCheckBoxes.length > 0) {
                ...
                // FIX: Pass recommendationWithFormfieldData explicitly
        ...
        if (!shouldSkipInterimAnswer) {
          ...
          if (
            ...
            if (interimRecommend.length > 0) {
              ...
              // FIX: recommendationWithFormfieldData is now local — no need to reset it
            }
          }
        }
      }
...
const calculateScoreHandler: NextApiHandler = async (req, res) => {
  ...
  try {
    ...
    if (!!req?.headers?.authorization) {
      ...
      // FIX: Use jwt.verify() instead of jwt.decode() to validate token signature
      try {
        ...
        parseHasuraClaims(decodedToken, accessToken);
      } catch {
