// FIXED: packages/client/services/platform-window-message.service.ts
//
// Bugs fixed:
// [CRITICAL] Line 1034-1035: postParentMessage used "*" as targetOrigin.
//   Any page embedding WARP in an iframe receives ALL messages regardless of origin.
//   Messages include accessTokens, formIds, invitationIds, and user action signals.
//   Fix: Use NEXT_PUBLIC_PARENT_ORIGIN env var; fall back to window.location.origin
//   so messages only reach the embedding page's known origin.
//
// [LOW] Line 197-203: Stray console.log in invitationFormStartMessage leaks
//   formId, InvitationId, AIData, formType to production browser console.
//
// [LOW] Line 1026: isFileField parameter ignored — hardcoded true in uploadFilesPopup.
//   Fix: use the isFileField parameter value.
//
// [LOW] warpAssignReviewer and warpAssignReviewerforAssessment are byte-for-byte
//   identical — one should be removed. Kept both for backwards compatibility
...
// FIX: Resolve the allowed parent origin once at module load time.
// NEXT_PUBLIC_PARENT_ORIGIN must be set in the deployment environment.
// Falls back to window.location.origin (same-origin) if not configured.
const getParentOrigin = (): string => {
  if (process.env.NEXT_PUBLIC_PARENT_ORIGIN) {
    return process.env.NEXT_PUBLIC_PARENT_ORIGIN;
  }
  if (typeof window !== "undefined") {
    return window.location.origin;
  }
  return "*"; // SSR fallback — postMessage won't be called server-side
};
...
export const invitationFormStartMessage = (
  ...
  // FIX: Removed console.log that leaked formId, InvitationId, AIData, formType
  // to the production browser console.
  return JSON.stringify({
...
// NOTE: warpAssignReviewerforAssessment is byte-for-byte identical to warpAssignReviewer.
// These should be consolidated into a single function.
...
export const uploadFilesPopup = (
  ...
  return JSON.stringify({
    ...
    data: {
      ...
      isFileField: isFileField, // FIX: was hardcoded `true`, now uses parameter value
      questionId: questionId,
...
// FIX: Use NEXT_PUBLIC_PARENT_ORIGIN as targetOrigin instead of "*".
// Previously any page embedding WARP in an iframe received all messages
// regardless of domain — this exposed access tokens, form IDs, and user
// action signals to any embedding origin.
