...
export const stz_string_tds = (val: string) => val?.trim().replace("  ", " ");
...
export const stz_string_tlms = (val: string) =>
  val?.replace(/\s+/g, " ").trim().toLocaleLowerCase();
export const stz_string_tls = (val: string) =>
  val?.replace(/\s+/g, "").toLocaleLowerCase();
export const stz_string_tl = (val: string) => val?.trim()?.toLocaleLowerCase();
/**
 * @description
