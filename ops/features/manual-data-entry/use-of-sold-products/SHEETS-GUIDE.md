...
- **Product Code** - Required, alphanumeric format; numeric-only values (e.g. `3452`) are accepted and converted to string before saving — the service uses `String(value).trim()` to guarantee a TEXT value reaches Hasura regardless of what type the Excel parser produces
...
- **Rationale** - Optional. Leading/trailing whitespace (spaces, tabs, newlines) is trimmed before validation. Whitespace-only values (e.g. multiple spaces or Enter presses) are treated as not provided and saved as `null`. Pure numeric values are rejected with `"Invalid Input: Numeric values are not allowed"`.
- **Additional comments** - Optional (preprocessed: numbers auto-converted to string)
- **Remarks** - Optional (preprocessed: numbers auto-converted to string)
- **Working details 1-5** - Optional, free text, max 5000 characters (preprocessed: numbers auto-converted to string)
