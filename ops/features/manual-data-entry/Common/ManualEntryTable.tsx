...
import EditIconDisabled from "~/components/icons/EditIconDisabled";
import {
  GRID_POWER_STATUS,
  UPLOAD_TYPES,
  UploadType,
...
const ManualEntryTable = ({
    ...
    (
      ...
      switch (col.type) {
        case "single-select": {
          ...
          // Open upward only for the last 2 rows AND only when the table has
          // more than 6 rows. For small tables we also disable Floating UI's
          // `flip` middleware — otherwise the iframe's short viewport makes
          // the dropdown clip at the bottom edge and auto-flip upward even
          // though there are very few rows.
          const totalRows = table.getRowModel().rows.length;
          const shouldOpenUpward = totalRows > 6 && row.index >= totalRows - 2;
          ...
          return (
            <Select
              ...
              withScrollArea={true}
              comboboxProps={{
                middlewares: { flip: shouldOpenUpward, shift: true },
                withinPortal: true,
        ...
        Edit: ({ cell, column, row, table }) => {
          ...
          switch (col.type) {
            case "single-select": {
              ...
              // Open upward only for the last 2 rows AND only when the table
              // has more than 6 rows. For small tables we also disable
              // Floating UI's `flip` middleware — otherwise the iframe's
              // short viewport makes the dropdown clip at the bottom edge
              // and auto-flip upward even with very few rows.
              const totalRows = table.getRowModel().rows.length;
              const shouldOpenUpward =
                totalRows > 6 && row.index >= totalRows - 2;
              const dropdownPosition = shouldOpenUpward
                ? "top-start"
                : "bottom-start";
  ...
  const table = useMantineReactTable({
    ...
    enableRowSelection:
      !isOrganizationAdmin && !hideCheckboxes
        ? (row) => {
            const hasAI = hasAIExtractedData
              ? hasAIExtractedData(row.original)
              : false;
            return (
              !hasAI &&
              (row.original as any)?.status !== GRID_POWER_STATUS.APPROVED
            );
          }
        : false,
    enablePinning: true,
    ...
    displayColumnDefOptions: {
      "mrt-row-select": {
        ...
        Cell: ({ row, table }) => {
          ...
          const hasAI = hasAIExtractedData && hasAIExtractedData(row.original);
          const isApproved =
            row.original?.status === GRID_POWER_STATUS.APPROVED;
          const isDisabled =
            hasAI ||
            isApproved ||
            !!table.getState().editingRow ||
    ...
    renderRowActions: ({ row, table }) => {
      ...
      if (enableInlineEditForPrePopulated) {
        ...
        const rowError = validationErrors[row.id]?._rowError as
          | string
          | undefined;
            ...
            <Flex gap={8} align="center">
              {isApprovedRow ? (
                ...
                <>
                  <Tooltip
                    label={
                      hasModifications ? "Save" : "Edit fields to enable save"
                    }
                  >
                    <ActionIcon
                      onClick={() =>
                        !isButtonsDisabled && handleSaveInlineRow(row)
                      }
                  ...
                  <Tooltip
                    label={
                      hasModifications
                        ? "Cancel"
                        : "Edit fields to enable cancel"
                    }
                  >
                    <ActionIcon
                      onClick={() =>
                        !isButtonsDisabled && handleCancelInlineEdit(row)
                      }
    ...
    renderTopToolbar: ({ table }) => {
      ...
      const handleDelete = async () => {
        ...
        if (onDeleteRows) {
          // const success = await onDeleteRows(selectedIndices);
          // if (success) {
          //   // Parent will handle data update, we just clear selection
          //   table.resetRowSelection();
          // }
