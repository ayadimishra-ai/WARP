...
import { getMaterialMasterUoMCategory } from "~/lib/uom-category";
import {
  MATERIAL_CODE,
  MATERIAL_NAME,
  MATERIAL_TYPE,
...
export class ScenarioUoMCategoryChangeBlockedRule extends MaterialMasterScenarioRule {
  ...
  async validate(
    ...
    const masterKeys = [
      validationContext.activityUomMasterKey ??
    ...
    return {
      errors,
      notifications: uomMismatches.length > 0 ? { uomMismatches } : undefined,
    };
  }
  ...
  private resolveCategory(
    ...
    masterKeys: string[],
    uomValue: string
  ): import("~/lib/uom-category").UoMCategory | null {
