...
export const fetchExistingTemplateGroups = async (
  ...
  const normalizedCodes = materialCodes.map((code) =>
    code.toLowerCase().trim()
  );

  const result = await sdk.getDistinctUOMsUpstreamCapitalGoodsByMaterialCodes({
    whereUpstream: {
  ...
  result?.GHGCapital_Goods?.forEach((item) => {
    ...
    templateGroupMap.get(code)!.add(TEMPLATE_GROUPS.CAPITAL_GOODS);
  });
  ...
  result?.GHGTransport_Upstream?.forEach((item) => {
    ...
    templateGroupMap.get(code)!.add(TEMPLATE_GROUPS.PROCUREMENT);
  });
   ...
   */
  const materialMasterResult = await sdk.getMaterialMasterByOrgIdAndCodes({
    where: {
      _and: [
        {
          _or: normalizedCodes.map((code) => ({
...
export const fetchMaterialProcurementUsage = async (
  ...
  const result = await sdk.getDistinctUOMsMaterialProcurementByMaterialCodes({
    organizationId,
...
export const validateCrossTemplateForBatch = async (
  ...
  const results = new Map<string, ICrossTemplateValidationResult>();
  ...
  const templateGroupMap = await fetchExistingTemplateGroups(
    materialCodes,
    organizationId
  );
  ...
  if (incomingGroup === TEMPLATE_GROUPS.CAPITAL_GOODS) {
    const procurementUsage = await fetchMaterialProcurementUsage(
      materialCodes,
      organizationId
    );
  ...
  for (const rawCode of materialCodes) {
    ...
    const existingGroups =
      templateGroupMap.get(normalizedCode) ?? new Set<TemplateGroup>();
...
export async function checkConflictsForCapitalGoods(
  ...
  const conflicts = new Map<string, ICrossTemplateConflict>();

  const validationResults = await validateCrossTemplateForBatch(
    materialCodes,
    "capital_goods",
    organizationId
  );
  ...
  validationResults.forEach((result, code) => {
    conflicts.set(code, {
      materialCode: code,
      conflictingTemplate: "Upstream / Material Procurement",
    });
  });
