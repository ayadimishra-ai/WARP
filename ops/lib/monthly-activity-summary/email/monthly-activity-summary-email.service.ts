...
import type { ApproveParams } from "~/lib/monthly-activity-summary/types";
...
import { uploadFileBufferToS3ForAI } from "~/utils/file-storage/server.service";
...
async function fetchLocationNames(
  ...
  // (PostgreSQL oa.id::text is always lowercase; client UUIDs may be mixed-case).
 ...
 * SQL logic is identical to fetchPendingCombosForUser; only the WHERE clause
 * changes from = '${userId}' to IN (${idList}) and user_id is added to SELECT.
 */
...
export interface NotifyOrgAdminsOnDataUploadParams {
  ...
  year?: number;
  /**
   * All distinct (year, month) pairs from the upload, sorted latest first.
