...
/** Returns the start-year of the current financial (or calendar) year. */
...
async function _getFiltersUncached(
  ...
  // Build year options from only the years that have at least one data record.
  // This prevents showing empty year periods in the dropdown.
  const years = buildYearOptions(dataYears, startMonth, yearType);

  // Default to the latest year that has real data; fall back to current FY.
  const fallbackYear = currentDefaultYear(startMonth, yearType);
  const defaultYear = latestDataYear ?? fallbackYear;
...
export async function getSummaryData(
  ...
  const rawRows = (await unstable_cache(
    // The function to cache — receives no args itself; all params are closed
    // over and encoded in the cache key below.
    () =>
      runSummaryQuery({
...
export async function approveActivity(
  ...
  return { approvedCount, approvedLocationIds };
}
