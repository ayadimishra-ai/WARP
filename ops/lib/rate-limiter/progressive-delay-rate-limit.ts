...
export const withEmailOrIpRateLimitWithProgressiveDelay = (
  handler: (request: NextRequest, userSession?: any) => Promise<NextResponse>,
  options?: {
  ...
  return async (request: NextRequest): Promise<NextResponse> => {
    ...
    // success responses after a 429. Use a composite key (email/IP + pathname)
    // so limits are applied per API endpoint rather than globally across all APIs.
    const _path = request.nextUrl?.pathname ?? new URL(request.url).pathname;
    const compositeKey = `${key}::${_path}`;
    return enqueueSerial(compositeKey, async () => {
      const now = Date.now();
      let store = requestStore.get(compositeKey);
      ...
      if (!store || now > store.resetTime) {
        ...
        requestStore.set(compositeKey, store);
      }
