...
| Event | Cache action |
|---|---|
| Data uploaded | `pending_count += N` for that (location, activity, year, month) |
| Data re-uploaded (soft-delete old ATR) | Decrement old row, increment new row |
| Admin approves | `pending_count -= N`, `approved_count += N` |
| Admin rejects | `pending_count -= N`, `rejected_count += N` |

All cache updates wrapped in the same DB transaction as the primary operation — no partial updates.
...
| Transaction failure | If primary write succeeds but cache update fails, counts are stale | Wrap in single transaction; fall back to live query on cache miss |
