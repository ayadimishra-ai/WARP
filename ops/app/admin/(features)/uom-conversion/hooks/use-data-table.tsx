...
import { z } from "zod";
import { URL_UOM_CONVERSION } from "~/app/admin/api/api-routes";
import { TGetUserConversionFactorsSuccess } from "~/app/admin/libs/common/types";
import {
  useOPFetcher_GET,
  useOPFetcher_POST,
...
export const useDataTable = () => {
  ...
  const fuelsSelectData = useMemo(() => {
    ...
    const { uomConversionFactors, fuelTypeMaster } = data.data;
    const fuelsSet = new Set<string>();
    if (fuelTypeMaster && fuelTypeMaster.length > 0) {
      fuelTypeMaster.forEach((d) => {
        if (d.label) {
          fuelsSet.add(d.label);
        }
      });
    }
    // if (uomConversionFactors && uomConversionFactors.length > 0) {
    //   uomConversionFactors.forEach((d) => {
    //     if (d.fuel) {
    //       fuelsSet.add(d.fuel);
    //     }
