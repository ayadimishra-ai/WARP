  ...
  return sql.raw(` 
	with cte_captive as (
		select 
			...
			gcpr."Unit_of_Energy_Generated_in_Kwh" as unit_renewable_kwh,
			gcpnr."Type_of_Fuel_Used",
			gcpnr."Unit_of_Energy_Generated_in_Kwh" as unit_nonrenewable_kwh
		from "GHGEnergy_CaptivePower" gcp 
		left join "GHGEnergy_CaptivePower_Renewable" gcpr on gcpr."GHGEnergyConsumption_CaptivePower_id" = gcp.id
		left join "GHGEnergy_CaptivePower_NonRenewable" gcpnr on gcpnr."GHGEnergyConsumption_CaptivePower_id" = gcp.id
		order by gcp.task_request_id 
	),
	cte_captive_renewable as (
		...
		from cte_captive ctc
		where ctc."Type_of_Captive_Power" = 'Renewable'
		order by ctc.task_request_id asc
	),
	cte_captive_nonrenewable as (
		select 
			...
			'captive' as "source",
			'captive' as contract_type,
			ctc."Type_of_Fuel_Used" as resource,
			ctc.unit_nonrenewable_kwh
		from cte_captive ctc
		where ctc."Type_of_Captive_Power" = 'Non Renewable'
		order by ctc.task_request_id asc
	),
	cte_union_captive as (
		select * from cte_captive_renewable
		union all
		select * from cte_captive_nonrenewable
	)
...
export const SQL_QUERY_GET_Category11_KPI_Details = (
  taskrequestlist: string
) => {
  return sql.raw(`
