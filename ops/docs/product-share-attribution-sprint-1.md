...
3. **address_id**
   ...
   - Type: `uuid`
   - Nullable: `false`
   - Description: Reference to the supplier location/facility
...
8. **supplier_code**
   ...
   - Type: `text`
   - Nullable: `false`
   - Description: Supplier code representing the facility
...
10. **buyer_material_procurement_quantity**
    ...
    - Nullable: `true`
    - Description: Quantity of material procured by the buyer

11. **buyer_material_procurement_uom**

    - Type: `text`
    - Nullable: `true`
    - Description: Unit of measure for procurement quantity
...
13. **kpi_allocated_em_Grid_Power**
    ...
    - Type: `double precision`
    - Nullable: `true`
    - Description: Allocated emissions from grid power consumption

14. **kpi_allocated_em_Captive_Power**

    - Type: `double precision`
    - Nullable: `true`
    - Description: Allocated emissions from captive power generation

15. **kpi_allocated_em_Fuel_Purchased**

    - Type: `double precision`
    - Nullable: `true`
    - Description: Allocated emissions from fuel consumption

16. **kpi_allocated_em_Waste_Generation**

    - Type: `double precision`
    - Nullable: `true`
    - Description: Allocated emissions from waste generated

17. **kpi_em_total_pcf_supplier_facility**

    - Type: `double precision`
    - Nullable: `true`
    - Description: Facility Emissions Per Unit (KgCO2e). Calculated as: `allocation_percentage × (kpi_allocated_em_Grid_Power + kpi_allocated_em_Captive_Power + kpi_allocated_em_Fuel_Purchased + kpi_allocated_em_Waste_Generation) / buyer_material_procurement_quantity`
...
CREATE TABLE "KPIProductCarbonFootprintSupplierFacility" (
  ...
  "kpi_em_total_pcf_supplier_facility" double precision,
  "em_uom" text,
  "metadata" jsonb,
  "is_deleted" boolean DEFAULT false,
  "created_at" timestamp with time zone DEFAULT now(),
...
15. **kpi_em_total_pcf_material_procurement**
...
"kpi_em_total_pcf_material_procurement" numeric,
"em_uom" text,
"metadata" jsonb,
"is_deleted" boolean DEFAULT false,
"created_at" timestamp with time zone DEFAULT now(),
...
14. **em_uom**
    ...
    - Type: `text`
    - Nullable: `true`
    - Description: Emission unit of measure for kpi_em_total_pcf_upstream (e.g., KgCO2e/Kilogram, KgCO2e/Litre, KgCO2e/EA, KgCO2e/Nos)
...
CREATE TABLE "KPIProductCarbonFootprintUpstream" (
  ...
  "kpi_em_total_pcf_upstream" numeric,
  "em_uom" text,
  "metadata" jsonb,
  "is_deleted" boolean DEFAULT false,
  "created_at" timestamp with time zone DEFAULT now(),
...
```
kpi_em_total_pcf_supplier_facility = allocation_percentage × (kpi_allocated_em_Grid_Power + kpi_allocated_em_Captive_Power + kpi_allocated_em_Fuel_Purchased + kpi_allocated_em_Waste_Generation) / buyer_material_procurement_quantity
```
...
```
kpi_em_total_pcf_upstream = kpi_em_upstream / buyer_material_procurement_quantity
em_uom = KgCO2e / {converted_quantity_uom}
```
...
- `em_uom` = e.g., KgCO2e/Kilogram, KgCO2e/Litre, KgCO2e/EA, KgCO2e/Nos
...
- kpi_em_total_pcf_upstream = 50,000 / 400 = 125 KgCO2e/EA
- em_uom = KgCO2e/EA
...
# Audit Log — ClickHouse Table: SupplierMaterialMapping

# Table Name - snowkap_op_logs.SupplierMaterialMapping

## Table Description

This table stores audit log entries for the `SupplierMaterialMapping` records in ClickHouse. It is written to by the `saveSupplierMaterialMapping` function in `lib/auditlog/auditlog.service.ts` using the `JSONEachRow` format with a batch size of 1000.

## Table Columns

1. **id**
   - Type: `UUID`
   - Description: Unique identifier of the mapping record (from `SupplierMaterialMapping.id`)
...
3. **supplier_address_mapping_id**
   - Type: `Nullable(UUID)`
   - Description: References `SupplierMaterialMapping.supplier_address_mapping_id`

4. **org_material_master_id**
   - Type: `Nullable(UUID)`
   - Description: References `SupplierMaterialMapping.org_material_master_id`

5. **From_Year**
   - Type: `Nullable(Int32)`
   - Description: From year of the mapping period

6. **From_Month**
   - Type: `Nullable(String)`
   - Description: From month of the mapping period (e.g. `January`)

7. **To_Year**
   - Type: `Nullable(Int32)`
   - Description: To year of the mapping period

8. **To_Month**
   - Type: `Nullable(String)`
   - Description: To month of the mapping period (e.g. `December`)

9. **meta_data**
   - Type: `Nullable(String)`
   - Description: JSON-stringified metadata from the source record
...
```sql
CREATE TABLE snowkap_op_logs.SupplierMaterialMapping
(
    `id`                          UUID,
    `organization_id`             UUID,
    `supplier_address_mapping_id` Nullable(UUID),
    `org_material_master_id`      Nullable(UUID),
    `From_Year`                   Nullable(Int32),
    `From_Month`                  Nullable(String),
    `To_Year`                     Nullable(Int32),
    `To_Month`                    Nullable(String),
    `meta_data`                   Nullable(String) COMMENT 'JSONB field',
