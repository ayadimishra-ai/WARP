...
function WorkStreamBlock(props: any) {
  ...
  const sumOverallWorkStreamBlock = () => {
    ...
    setChartDataPie([
      {
        category: "Upstream",
        value: yearQuarter?.kpi_em_Cont_TotalEmission_StreamOfWork_Upstream,
      },
      {
        category: "Operations",
        value: yearQuarter?.kpi_em_Cont_TotalEmission_StreamOfWork_Operations,
      },
      {
        category: "Downstream",
        value: yearQuarter?.kpi_em_Cont_TotalEmission_StreamOfWork_Downstream,
      },
    ]);
  };
  useMemo(() => {
  ...
  useEffect(() => {
    const data = getStoreData?.currentYear?.main?.map((item: any) => {
      let data = {
        ...
        year: item.year,
        month: item.month,
        downstream: item.kpi_em_Cont_TotalEmission_StreamOfWork_Downstream,
        operations: item.kpi_em_Cont_TotalEmission_StreamOfWork_Operations,
        upstream: item.kpi_em_Cont_TotalEmission_StreamOfWork_Upstream,
      };
      return data;
    });
  ...
  if (emission_contributors && emission_contributors.length > 0) {
    // For Contribution in Total Emission by Work Streams
    const totalEmissionSnapshot: EmissionContributor | undefined =
      emission_contributors.find(
        (desc: EmissionContributor) =>
          desc.category === "Contribution in Total Emission by Work Streams"
      );
  ...
  const chartDataMemo = useGetGroupByChartData(
    chartDataLine,
    "Contribution in Total Emission by Work Streams(in tco2e)"
  );

  return (
    <Card ref={chartRef} padding="md" radius="md" h="100%">
      <ChartHeadBlock
        title="Contribution in Total Emission by Work Streams"
        unit={`in ${unit}`}
        ...
        DownloadChart={() =>
          DownloadChart(
            chartRef,
            "Contribution in Total Emission by Work Streams",
            "png"
          )
        }
        downloadPDF={() =>
        ...
        DownloadCSV={() =>
          DownloadCSV<Record<string, any>>(
            ...
            [
              "Contribution in Total Emission by Work Streams - PieChart",
              "Contribution in Total Emission by Work Streams - LineChart",
            ],
            [true]
          )
        }
...
export default WorkStreamBlock;
