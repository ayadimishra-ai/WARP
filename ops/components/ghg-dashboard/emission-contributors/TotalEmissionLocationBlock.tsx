...
function TotalEmissionLocationBlock(props: any) {
  ...
  const sumOverallLocation = () => {
    const grouped = _.groupBy(getStoreData?.currentYear?.main, "region_id");
    const regionPieChartData = _.map(grouped, (values, key) => {
      return {
        category: values[0]?.Region?.name,
        value: _.sumBy(values, "kpi_em_Total_Emission"),
      };
    });

    setChartDataPie(regionPieChartData);
  };
  ...
  useEffect(() => {
    const data = getStoreData?.currentYear?.main?.map((item: any) => {
      return {
        year: item.year,
        month: item.month,
        [!!item?.Region && item?.Region?.name]: item.kpi_em_Total_Emission,
      };
    });
  ...
  const chartDataMemo = useGetGroupByChartData(
    chartDataLine,
    "Contribution in Total Emissions by Regions(in tco2e)"
  );

  return (
    <Card ref={chartRef} padding="md" radius="md" h="100%">
      <ChartHeadBlock
        title="Contribution in Total Emissions by Regions"
        unit={`in ${unit}`}
        ...
        DownloadChart={() =>
          DownloadChart(
            chartRef,
            "Contribution in Total Emissions by Regions",
            "png"
          )
        }
        downloadPDF={() =>
        ...
        DownloadCSV={() =>
          DownloadCSV<Record<string, any>>(
            ...
            [
              "Contribution in Total Emissions by Regions - PieChart",
              "Contribution in Total Emissions by Regions - LineChart",
            ],
            [true]
          )
        }
...
export default TotalEmissionLocationBlock;
