...
import { GhgEnergyConsumption_FuelPurchased_General_Insert_Input } from "@/modules/ghg/graphql/shared/types";
import {
...
import { validateExcelTemplateData } from "@/modules/ghg/lib/organization-transaction/energy/energy-fuel-purchased.validation";
...
async function updateFuelConsumptionGeneral(
  ...
  // Keep the parent's updated_by in sync so the info tooltip shows the correct editor
  await sdk.updateFuelPurchasedParentUpdatedBy({
    id: parentGhgIdToUse as string,
    updated_by: session.userId,
    updated_at: now,
  });
