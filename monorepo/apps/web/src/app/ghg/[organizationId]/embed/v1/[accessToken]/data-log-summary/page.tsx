...
type TUrlFilters = {
   ...
   * when yearMonthPairs is absent (legacy payloads).
   */
  months?: string[];
  year?: number;
  /**
   * Per-(year, month) pairs from the upload (added in newer email payloads).
   * yearMonthPairs[].year is the CALENDAR year straight from the uploaded file.
   * When present this takes precedence over the year + months[0] anchor approach;
   * each pair's calendar year is converted to the FY start year here (via
   * toFyStartYear) so multi-year / cross-FY-boundary uploads resolve correctly.
   */
...
const DataUploadLogSummary: React.FC = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState<"pending" | "approved" | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchFilters = async () => {
      ...
      try {
        const res = await apiClientWithAuth.get("/api/v1/monthly-activity-summary/filters");
        ...
        if (json.success) {
          ...
          const toBadges = (months: string[]) =>
            months
              .map((m) => MONTH_FULL_TO_BADGE[m.charAt(0).toUpperCase() + m.slice(1)] ?? null)
              .filter((b): b is string => !!b);

          // ── Year + Months ──
          // Two strategies depending on which filter payload version was sent:
          //
          // New payloads (yearMonthPairs present): each pair carries the FY start
          // year directly — find the max FY year and pre-select only the months
          // that belong to it. Handles multi-year uploads without ambiguity.
          //
          // Legacy payloads (year + months only): convert the calendar year to an
          // FY start year using months[0] as the anchor (encoder sorts latest-first).
          const urlYearMonthPairs = (urlFilters?.yearMonthPairs ?? []).filter(
            (p) => p && typeof p.year === "number" && typeof p.month === "string"
          );
          const urlMonths =
          ...
          let resolvedMonthNames: string[];

          if (urlYearMonthPairs.length > 0) {
            // New path: each pair carries a CALENDAR (year, month). Convert each
            // to its financial-year start year (using the org's year type + FY
            // start month) before bucketing, then pick the latest FY year and
            // pre-select every month that falls in it. Without this conversion a
            // cross-FY-boundary upload (e.g. Dec 2023 + Jan 2024, both in FY2023
            // for an April FY) would split across FY2023/FY2024 and resolve to the
            // wrong year while dropping a month.
            const fyPairs = urlYearMonthPairs.map((p) => ({
              fyYear: toFyStartYear(
                p.year,
                p.month,
                data.yearType,
            ...
            const maxFyYear = Math.max(...fyPairs.map((p) => p.fyYear));
            resolvedYear = String(maxFyYear);
            resolvedMonthNames = fyPairs
              .filter((p) => p.fyYear === maxFyYear)
              .map((p) => p.month);
          } else if (urlFilters?.year && urlFilters.year > 0) {
            // Legacy path: derive FY year from calendar year + anchor month.
            ...
            resolvedYear = String(fyYear);
            resolvedMonthNames = urlMonths;
          } else {
            resolvedYear = String(data.defaultYear);
            resolvedMonthNames = [];
          }
          setSelectedYear(resolvedYear);

          // ── Months ──
          if (resolvedMonthNames.length > 0) {
            const badges = toBadges(resolvedMonthNames);
          ...
          if (urlFilters?.organizationAddressId) {
            resolvedLocations = [urlFilters.organizationAddressId];
          } else {
            resolvedLocations = data.locations.map((l) => l.id);
          }
          setSelectedLocations(resolvedLocations);
          ...
          // ── monthsWithData ──
          // When URL filters drive a non-default year or a specific location,
          // re-fetch /filters with those params so month tiles reflect data for
          // that exact year/location — the same call that onLocationsChange and
          // onFinancialYearChange make for manual filter changes.
          const hasUrlOverride =
            urlYearMonthPairs.length > 0 ||
            !!(urlFilters?.year && urlFilters.year > 0) ||
            !!urlFilters?.organizationAddressId;
          if (hasUrlOverride) {
            try {
              const locationParam = resolvedLocations.join(",");
              const scopedRes = await apiClientWithAuth.get(
                `/api/v1/monthly-activity-summary/filters?year=${resolvedYear}&locationIds=${locationParam}`
              );
              const scopedJson = await scopedRes.data;
              setMonthsWithData(
                scopedJson.success
                  ? (scopedJson.data.monthsWithData ?? [])
                  : (data.monthsWithData ?? [])
              );
            } catch {
  ...
  const fetchSummaryData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiClientWithAuth.post("/api/v1/monthly-activity-summary", body);
      const json = await res.data;
  ...
  const createStatusColumn = <T extends TActivityRow | TLocationRow>(
    ...
    return {
      ...
      ...(includeFooter && { mantineTableFooterCellProps: { align: "center" } }),
      Header: () => (
      ...
      try {
        const body = {
          ...
          months: selectedMonths
            .map((badge) =>
              (MONTH_BADGE_TO_FULL[badge] ?? badge).toLowerCase()
            )
            .filter(Boolean),
        }
        const res = await apiClientWithAuth.post("/api/v1/monthly-activity-summary/approve", body);
        const json = await res.data;
        if (json.success) {
  ...
  const handleExport = useCallback(
    ...
    [exportingKeys, selectedYear, filterData?.yearType, selectedMonths, selectedStatus]
  );
  ...
  const createActionsColumn = <T extends TActivityRow | TLocationRow>(
    ...
    Cell: ({ row }) => {
      ...
      return (
        <Flex gap={12} justify="flex-end" align="center" direction="row" pr={isOrgAdmin ? 15 : 34}>
            ...
            >
              ...
              >
                <IconCheck
                  size={24}
                  color={
                    noRecords ? "#adb5bd" : (allApproved || selectedStatus === "approved") ? "#00a63e" : "#003B52"
                  }
                />
              </ActionIcon>
            </Tooltip>
    ...
    ...(includeFooter && {
      Footer: () => (
        <Text fw={700} pl={(activeTab === "activity_type" && isOrgAdmin) ? 30 : 10}>
          -
        </Text>
      ),
    }),
  ...
  const activityTypeColumns = useMemo<MRT_ColumnDef<TActivityRow>[]>(
    () => [
          ...
          return (
            <Tooltip label={value} position="top" withArrow openDelay={300}>
              <Flex align="center" gap={8} w="max-content">
                <Paper styles={{ root:{...dotStyle("#005c81"), flexShrink: 0} }} />
                <Text fz={14} c="#122f47">
                  {value}
                </Text>
              </Flex>
      ...
      ...(selectedStatus === null || selectedStatus === "pending"
        ? [createStatusColumn("pending", pendingColors) as MRT_ColumnDef<TActivityRow>]
        : []),
      // Approved column — shown when no filter or approved filter is active
      ...(selectedStatus === null || selectedStatus === "approved"
        ? [createStatusColumn("approved", approvedColors) as MRT_ColumnDef<TActivityRow>]
        : []),
  ...
  const locationWiseColumns = useMemo<MRT_ColumnDef<TLocationRow>[]>(
    () => [
      ...
      // Pending column — shown when no filter or pending filter is active
      ...(selectedStatus === null || selectedStatus === "pending"
        ? [createStatusColumn("pending", pendingColors) as MRT_ColumnDef<TLocationRow>]
        : []),
      // Approved column — shown when no filter or approved filter is active
      ...(selectedStatus === null || selectedStatus === "approved"
        ? [createStatusColumn("approved", approvedColors) as MRT_ColumnDef<TLocationRow>]
        : []),
      ...
      {isLoadingFilters ? (
        ...
        <MainFilterBlock
          ...
          onLocationsChange={(vals) => {
            ...
            apiClientWithAuth
              .get(`/api/v1/monthly-activity-summary/filters?year=${yearParam}&locationIds=${locationParam}`)
              .then((r) => r.data)
              .then((json) => {
                if (json.success) setMonthsWithData(json.data.monthsWithData ?? []);
              })
              .catch(() => {});
          }}
          ...
          onFinancialYearChange={(val) => {
            ...
            if (val) {
              ...
              apiClientWithAuth.get(`/api/v1/monthly-activity-summary/filters?year=${val}&locationIds=${locationParam}`)
                .then((r) => r.data)
                .then((json) => {
                  if (json.success) {
