...
const inputValidationSchema = z.object({
  organizationAddressId: z
    .string()
    .min(1, "organization address id is required")
    .uuid("Invalid organization address id"),
  files: z
    .array(z.custom<File>())
...
const postHandler = async (req: NextRequest, userSession: TUserSession) => {
  ...
  const formDataPayload = {
    organizationAddressId: formdata.get("organizationAddressId") ?? "",
  ...
  const { files, organizationAddressId } = parsedData.data;
  const { organizationId } = userSession || {};
  const sdk = await getGraphQlServerSDK();
  try {
    const organizationAddress = await sdk.getOrganizationAddressById({
      organizationAddressId,
    });

    if (!!!organizationAddress?.OrganizationAddress?.length) {
      throw CustomError({
        statusCode: 400,
