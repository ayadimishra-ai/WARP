...
import { saveESGGrievances } from "@/modules/ghg/lib/auditlog/auditlog.service";
...
import { insertGrievancesTemplateData } from "@/modules/ghg/lib/organization-transaction/grievances/grievances-excel.service";
import {
    validateGrievancesExcelTemplate,
    validateGrievancesExcelTemplateData,
} from "@/modules/ghg/lib/organization-transaction/grievances/grievances-excel.validation";
...
import { GrievancesActivityConstant } from "@/modules/ghg/shared/constants/activity.constant";
...
async function postHandler(req: NextRequest, userSession: TUserSession) {
  ...
  await validateUserActivityAndOrganizationAddressPermissions(
    ...
    GrievancesActivityConstant.parent_code
  );
  // read excel data
  ...
  let validationErrors: Record<string, any>[] =
    validateGrievancesExcelTemplate(excelData);
  ...
  if (!!validationErrors?.length) {
    ...
    const historyData = await insertNewDataImportHistory(
      userSession,
      GrievancesActivityConstant.code,
      "Excel",
      fileName,
      input.fileUrl,
      "failure",
  ...
  let datavalidationerrors = await validateGrievancesExcelTemplateData(
    excelData,
  ...
  let saveresponse;
  try {
    saveresponse = await insertGrievancesTemplateData(
      excelData,
      GrievancesActivityConstant.parent_code,
    ...
    const lockResponse = await handleApprovalLockError(err, {
      ...
      activityCode: GrievancesActivityConstant.code,
  ...
  saveESGGrievances(
    saveresponse?.insert_ESGGrievances?.returning,
    userSession,
    saveresponse?.delete_ESGGrievances?.returning
  );
  if (!!saveresponse) {
    ...
    const historyData = await insertNewDataImportHistory(
      ...
      null,
      input.organizationAddressId,
      GrievancesActivityConstant.name,
      uploadMonths,
      uploadYear
    );
    ...
    upsertCacheForActivity({
      ...
      organizationAddressId: input.organizationAddressId,
      activityCode: "grievances_activity",
