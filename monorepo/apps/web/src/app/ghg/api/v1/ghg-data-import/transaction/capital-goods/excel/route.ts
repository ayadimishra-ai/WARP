...
import { saveGHGCapitalGoods } from "@/modules/ghg/lib/auditlog/auditlog.service";
import { TUserSession } from "@/modules/ghg/lib/auth/auth.client";
import { handleApprovalLockError } from "@/modules/ghg/lib/bulk-upload/bulk-upload-approval-error.handler";
import { saveCapitalGoodsSheetEntries } from "@/modules/ghg/lib/capital-goods/capital-goods-excel.service";
import {
    validateExcelTemplate,
    validateExcelTemplateData,
} from "@/modules/ghg/lib/capital-goods/capital-goods-excel.validation";
...
import { sendEmailForMaterial } from "@/modules/ghg/lib/organization-transaction/material-procurement/material-procurement-excel.service";
import { withEmailOrIpRateLimitWithProgressiveDelay } from "@/modules/ghg/lib/rate-limiter/progressive-delay-rate-limit";
import { generateAndUploadMissingEmissionFactorsFile } from "@/modules/ghg/lib/supplier-master/supplier-master.service";
import { validateUserActivityAndOrganizationAddressPermissions } from "@/modules/ghg/lib/user/user.validation";
import { CapitalGoodsActivityConstant } from "@/modules/ghg/shared/constants/activity.constant";
...
async function postHandler(req: NextRequest, userSession: TUserSession) {
  ...
  if (!!saveResponse) {
    ...
    return NextResponse.json({
      success: true,
      data: historyData,
      warnings: dataValidationErrors.warningEntries.length > 0
