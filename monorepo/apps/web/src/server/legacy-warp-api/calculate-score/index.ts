  ...
  for (let i = 0; i < eachobjectitem.length; i++) {
    ...
    if (
      ...
      const singlerecommendations: any = !!formfield?.recommendationCalc
        ?.recommendation
        ? (await jsonata(formfield?.recommendationCalc?.recommendation).evaluate(
            datas
          )) ?? []
        : [];
  ...
  try {
    ...
    const calculateChildrenScore = async (
      ...
      if (!!currentSection.Questions.length) {
        for (let k = 0; k < currentSection.Questions.length; k++) {
          ...
          const score = !!question.calc?.expression
            ? (await jsonata(question.calc.expression).evaluate(_questionScoreInput)) ??
              0
            : 0;
          ...
          const recommendations = !!question.calc?.recommendation
            ? (await jsonata(question.calc.recommendation).evaluate(
                _questionScoreInput
              )) ?? []
            : [];
                ...
                for (let m = 0; m < allCheckBoxes.length; m++) {
                  if (!!allCheckBoxes[m].displayRules) {
                    const isTrue = Boolean(
                      await jsonata(allCheckBoxes[m].displayRules[0].rule).evaluate(
                        _questionScoreInput
                      )
                    )
                      ? true
                  ...
                  if (currentInterimData.length == 0) {
                    ...
                    if (!!allCheckBoxes[g].displayRules) {
                      isManipulateInterimAnswer = Boolean(
                        await jsonata(allCheckBoxes[g].displayRules[0].rule).evaluate(
                          _questionScoreInput
                        )
                      )
                        ? true
      ...
      if (!!currentSection.children.length) {
        for (const m of currentSection.children) {
          await calculateChildrenScore(m);
        }
      }
      const score = !!currentSection.calc?.expression
        ? (await jsonata(currentSection.calc.expression).evaluate(sectionScoreInput)) ??
          0
        : 0;

      // console.log(JSON.stringify({ currentSection, sectionScoreInput }));
      const recommendations = !!currentSection.calc?.recommendation
        ? (await jsonata(currentSection.calc.recommendation).evaluate(
            sectionScoreInput
          )) ?? []
        : [];
      const id = formResult.find(
    ...
    for (let b = 0; b < treeMapSections.length; b++) {
      await calculateChildrenScore(treeMapSections[b]);
    }

    // Calculate score for form
    const score = !!form.calc?.expression
      ? (await jsonata(form.calc.expression).evaluate(sectionScoreInput)) ?? 0
      : 0;
    const recommendations = !!form.calc?.recommendation
      ? (await jsonata(form.calc.recommendation).evaluate(sectionScoreInput)) ?? []
      : [];
    ...
    await Promise.all(
      triggerResult[0].data.map(async (job: any) => {
        if (formId === job?.formId) {
          const scoreData: any = await jsonata(job?.transformExp).evaluate(
            sectionScoreInput
          );
          ...
          try {
            ...
            console.error(`Error triggering webhook for job ${job?.formId}:`, error);
            throw error; // Fail fast for Promise.all
          } finally {
