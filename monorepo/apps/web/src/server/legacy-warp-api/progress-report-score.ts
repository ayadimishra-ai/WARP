...
import { InterimFormLogs_Insert_Input } from "@/modules/warp/packages/graphql/generated/types";
...
const calculateScore = async (
  ...
  const calculateChildrenScore = async (
    ...
    if (!!currentSection.Questions.length) {
      for (const question of currentSection.Questions) {
        ...
        const score = !!question.calc?.expression
          ? (await jsonata(question.calc.expression).evaluate(_questionScoreInput)) ?? 0
          : 0;
    ...
    const score = !!currentSection.calc?.expression
      ? (await jsonata(currentSection.calc.expression).evaluate(sectionScoreInput)) ?? 0
      : 0;
    sectionScoreInput[currentSection.key] = score;
  };

  for (const section of treeMapSections) {
    await calculateChildrenScore(section);
  }
