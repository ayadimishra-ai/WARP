...
async function processIngestingSingle(data: {
  ...
  try {
    const dlRes = (await sdk.GetDocumentLogsWithSources?.({
      where: {
        ...
        status: {
          _in: [
            DocumentLogsStatusServer.Uploaded,
            DocumentLogsStatusServer.Processing,
            DocumentLogsStatusServer.Processed
          ]
        },
    ...
    if (
      ...
      try {
        ...
        if (previousInvitations.length > 0) {
          ...
          const allPreviousReports = previousReportResult?.DocumentLogs ?? [];
  ...
  const finalIngestPayload = [
    {
      ...
      files: insertedSourcesFromDb.map((s: any) => {
        ...
        return {
          ...
          is_processed: documentLog
            ? documentLog.status === DocumentLogsStatusServer.Processed ||
              documentLog.status === DocumentLogsStatusServer.Processing
            : false
        };
      })
    }
  ...
  const resp = await apiRequest.post(
    NEXT_PUBLIC_AIAPI_BASE_URL + "embeddings/ingest-files",
    finalIngestPayload
  );
...
const triggerOPSToIQCurationForInvitation = async (invitation: {
  ...
  payload: any;
  opsToIQId: string | null;
  error?: string;
} | null> => {
  try {
    if (!invitation || !invitation.invitationFormId) return null;

    // Build the insert row. triggeredByUserId is `uuid` in Hasura — sending an
    // empty string fails the mutation with "invalid input syntax for type uuid:
    // \"\"" and silently leaves no row in the table. Only include it when we
    // actually have a UUID-shaped value; otherwise let the column be null.
    const triggeredByUserId =
      invitation.created_by && invitation.created_by.trim().length > 0
        ? invitation.created_by
        : undefined;

    const insertRow: OpsToIqCuration_Insert_Input = {
      formInvitationId: invitation.invitationFormId,
      status: FormInvitationStatus.Processing as any,
      startAt: new Date().toISOString(),
    ...
    try {
      ...
      // graphql-request throws a ClientError with a `response.errors` array.
      // Surface the underlying Hasura message so the caller can return a
      // useful 500 instead of an opaque "Tracking record creation failed".
      ...
      console.error("OPSToIQCuration insert failed:", insertError, err);
    ...
    if (!matching?.id) {
      console.error(
        ...
        insertError ? `(insert error: ${insertError})` : ""
      );
      return {
        payload: null,
        opsToIQId: null,
        error: insertError ?? "Tracking row not returned by Hasura"
      };
    }
...
const AIprocessing: NextApiHandler = async (req, res) => {
  ...
  const processType = parsedBody.process;
  let requestBody = parsedBody.body;

  if (!processType) {
    return res.status(400).json({
      success: false,
      error: "Missing 'process' field in request body",
      message:
        "AIprocessing requires a JSON body with a 'process' field (e.g. fileParsing, opsToIQCuration, etc.)"
    });
  }
    ...
    switch (processType) {
      case "newFormInvitation": {
        ...
        response = await apiRequest.post(
          NEXT_PUBLIC_AIAPI_BASE_URL + "new-form-invitation",
          payloadToSend
        );
        break;
      }
      ...
      case "dataCurationStatus":
        response = await apiRequest.post(
          NEXT_PUBLIC_AIAPI_BASE_URL + "web-data-curation-status",
          req.body.data
        );
        break;
      ...
      case "deleteFile":
        response = await apiRequest.post(
          NEXT_PUBLIC_AIAPI_BASE_URL + "delete-upload-file",
          req.body.data
        );
        break;
      case "fileParsing": {
      ...
      case "chatQuery": {
        ...
        // Get AI service base URL + auth token from WARP AWS secret
        const warpSecrets = await secretsManagerService.getAllSecrets();
        const aiServiceBaseUrl = warpSecrets["NEXT_PUBLIC_AIAPI_BASE_URL"];
        if (!aiServiceBaseUrl) {
          console.error(
            "NEXT_PUBLIC_AIAPI_BASE_URL not found in WARP secret 'snowkap-warp-live'"
          );
        ...
        const headers: Record<string, string> = {
          ...
          "x-ai-services-authorization":
            warpSecrets["AI_SERVICES_AUTHORIZATION"] ?? ""
        };
        ...
        if (!aiResponse.ok) {
          ...
          return res.status(aiResponse.status).json({
            ...
            error:
              "AI Service is not available at present. Please try again after some time."
          });
        }
      ...
      case "opsToIQCuration": {
        ...
        // Get AI service base URL + auth token from AWS Secrets Manager
        const warpSecrets = await secretsManagerService.getAllSecrets();
        const aiServiceBaseUrl = warpSecrets["NEXT_PUBLIC_AIAPI_BASE_URL"];
        if (!aiServiceBaseUrl) {
        ...
        if (!curationData || !curationData.payload) {
          return res.status(500).json({
            success: false,
            message: "Failed to initialize OPS-to-IQ curation tracking",
            error:
              curationData?.error ??
              "Tracking record creation failed (no Hasura error captured)"
          });
        }
      ...
      case "deleteParsedDocument": {
        ...
        if (
          !document_log_ids ||
          !Array.isArray(document_log_ids) ||
          document_log_ids.length === 0
        ) {
          return res.status(400).json({
            success: false,
            message: "Missing required fields",
        ...
        console.log(
          `[Background] Initiating AI delete embeddings service for documents: ${document_log_ids.join(", ")}`
        );
