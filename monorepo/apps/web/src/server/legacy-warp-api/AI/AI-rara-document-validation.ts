...
import { raraDocumentValidation } from "@/modules/warp/packages/server/services/AI/AI-rara-document-validation";
import { NextApiHandler, NextApiRequest, NextApiResponse } from "next";
const handler: NextApiHandler = async (
  req: NextApiRequest,
  res: NextApiResponse
) => {
  const responseData = await raraDocumentValidation(req.body);
  if (!!responseData) {
