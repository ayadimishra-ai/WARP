...
import { auditLogFileUpload } from "@/app/ghg//actions/auditlog";
import { useUpdateAiFileUploadsMutation } from "@/modules/ghg/graphql/mutations/update-ai-uploads-status.generated";
import { useGetUploadedFilesQuery } from "@/modules/ghg/graphql/queries/get-ai-files-uploads.generated";
import { useGetOrganizationAddressAndActivityMappingQuery } from "@/modules/ghg/graphql/queries/get-organization-address-and-activity-mapping.generated";
import {
  AIFileUploadStatus,
  AIFileUploadStatusTooltip,
} from "@/modules/ghg/shared/constants/ai-constant";
import { ACTIVITY_CODES } from "@/modules/ghg/lib/shared/constants/dataimporthistory.constant";
import { viewVerifyExtractedData } from "@/modules/ghg/shared/services/platform-window-message-service";
import {
  AIFileUpload,
  SectionFilter,
  TCustomFilter,
} from "@/modules/ghg/shared/types/ai-types";
import { formatDateDisplay, handleDownload } from "@/modules/ghg/utils/common-functions";
import {
...
// Sentinel value used in the Location filter dropdown to represent records
// without an associated location (location_names is null or empty).
// Specific to the energy_grid_power AI history table.
const UNVERIFIED_LOCATION_VALUE = "__UNVERIFIED_LOCATION__";
const UNVERIFIED_LOCATION_LABEL = "Unverified Location";
...
const AIDataImportHistoryTable: React.FC = () => {
  ...
  const buildVariables = () => {
    ...
    // `allLocationsWhere` mirrors the role-based access clause but intentionally
    // OMITS the location filter and global search filter so the Location
    // dropdown can list every location the user has access to — independent of
    // pagination and of the current filter selection.
    let allLocationsWhere: any = {};
    const includesUnverifiedLocation = selectedLocation.includes(
      UNVERIFIED_LOCATION_VALUE
    );
    const namedLocations = selectedLocation.filter(
      (loc) => loc !== UNVERIFIED_LOCATION_VALUE
    );
    const locationOrConditions: any[] = [
      ...namedLocations.map((loc) => ({
        location_names: { _ilike: `%${loc}%` },
      })),
      ...(includesUnverifiedLocation
        ? [
            {
              _or: [
                { location_names: { _is_null: true } },
                { location_names: { _eq: "" } },
              ],
            },
          ]
    ...
    if (isAdmin) {
      ...
      allLocationsWhere = { _and: [{ is_deleted: { _eq: false } }] };
    }
  ...
  useEffect(() => {
    // Source of truth for the Location dropdown: the dedicated
    // `AIFileUploadListingView_AllLocations` field, which is NOT paginated and
    // NOT filtered by selectedLocation/global search — so every location the
    // current user has access to appears, regardless of which page or filter
    // the table is currently showing.
    const allLocationsSource = (data as any)
      ?.AIFileUploadListingView_AllLocations;
    if (!allLocationsSource) return;

    const uniqueLocations = Array.from(
      new Set(
        allLocationsSource.flatMap((item: any) => {
    ...
    // Accumulate locations across refetches. Without this, narrowing the
    // filter (e.g. selecting only "Unverified Location") returns rows with no
    // location_names, which would otherwise wipe the previously-seen named
    // locations out of the dropdown.
    setAllLocations((prev) => {
      const prevNamed = prev.filter(
        (l) => l.value !== UNVERIFIED_LOCATION_VALUE
      );
      const known = new Set(prevNamed.map((l) => l.value));
      const additions = uniqueLocations
        .filter((loc) => !known.has(loc))
        .map((loc) => ({ value: loc, label: loc }));
      // Always pin "Unverified Location" at the end of the list so the
      // energy_grid_power table can filter rows with no assigned location.
      return [
        ...prevNamed,
        ...additions,
        { value: UNVERIFIED_LOCATION_VALUE, label: UNVERIFIED_LOCATION_LABEL },
      ];
    });
    ...
    if (!hasPreSelectedRef.current) {
      ...
      setSelectedLocation([...uniqueLocations, UNVERIFIED_LOCATION_VALUE]);
    }
  }, [(data as any)?.AIFileUploadListingView_AllLocations]);
