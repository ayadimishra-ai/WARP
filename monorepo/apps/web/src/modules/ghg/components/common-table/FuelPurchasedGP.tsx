...
import { useGetGhgEnergyFuelPurchasedGeneralLazyQuery } from "@/modules/ghg/graphql/queries/get-ghgform-energyfuelpurchased-general-tabledata.generated";
import classes from "./CSS.module.css";
config.autoAddCss = false;
type FuelPurchasedToGenerateEnergyGP = {
  type_of_fuel_purchased: string;
  quantity_of_fuel_consumed: number;
  quality_of_fuel: string;
  point_of_consumption: string;
};
...
const data: FuelPurchasedToGenerateEnergyGP[] = [
  {
    type_of_fuel_purchased: "CNG",
    quantity_of_fuel_consumed: 1999,
    quality_of_fuel: "Non-Corrosive",
    point_of_consumption: "DG Set",
  },
  {
    type_of_fuel_purchased: "CNG",
    quantity_of_fuel_consumed: 1999,
    quality_of_fuel: "Non-Corrosive",
    point_of_consumption: "Direct",
  },
  {
    type_of_fuel_purchased: "CNG",
    quantity_of_fuel_consumed: 1999,
...
const FuelPurchasedGenerateEnergyGeneralPurposeTable = () => {
  ...
  const [data, setData] = useState<FuelPurchasedToGenerateEnergyGP[]>([]);
  const [rowCount, setRowCount] = useState(0);
  const [isError, setIsError] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [isRefetching, setIsRefetching] = useState(false);
  const [fetchData] = useGetGhgEnergyFuelPurchasedGeneralLazyQuery();
  ...
  useEffect(() => {
    const fetchDataFn = async () => {
      ...
      if (columnFilters?.length > 0 || !!globalFilter) {
        ...
        let point_of_consumption = columnFilters.filter(
          (d) => d.id === "point_of_consumption"
        )[0]?.value as string;
          ...
          _and: {
            ...
            ...(!!point_of_consumption &&
              point_of_consumption !== "" && {
                ...{
                  Point_of_Consumption: {
                    _ilike: `%${point_of_consumption.trim()}%`,
                  },
                },
              }),
      ...
      if (sorting?.length > 0) {
        ...
        if (sort.id === "point_of_consumption") {
          orderBy = {
            Point_of_Consumption: desc === true ? "desc" : "asc",
          };
        }
      }
      try {
        await fetchData({
          ...
          if (!!response.data) {
            const GHGEnergyConsumption_FuelPurchased_General: any = [];
            response?.data?.GHGEnergyConsumption_FuelPurchased_General?.map(
              (dbDetail: any) => {
                GHGEnergyConsumption_FuelPurchased_General.push({
                  ...
                  point_of_consumption: dbDetail?.Point_of_Consumption,
                });
              }
            );
            setData(GHGEnergyConsumption_FuelPurchased_General);
  ...
  const columns = useMemo<MRT_ColumnDef<FuelPurchasedToGenerateEnergyGP>[]>(
    () => [
      {
      ...
      {
        accessorKey: "point_of_consumption",
        header: "Point of Consumption",
      },
    ],
    []
  );
  ...
  const table = useMantineReactTable({
    renderTopToolbarCustomActions: () => (
      <Group className="sora-font " pt={6} gap={10} mt={5}>
        ...
        >
          Fuels Purchased to Generate Energy for General Purpose
        </Text>
...
export default FuelPurchasedGenerateEnergyGeneralPurposeTable;
