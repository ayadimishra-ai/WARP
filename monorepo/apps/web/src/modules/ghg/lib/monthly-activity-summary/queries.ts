/**
 * @Note Vishal Chhadekar 2025-04-21: Do not remove or delete this file. 
 */
...
export async function approveActivityTaskRequests(
  ...
  if (dataTables.length === 0) return { approvedCount: 0, approvedLocationIds: [] };
  ...
  const approvedLocationIds = [...new Set(atrRows.map((r) => String(r.organization_address_id)))];

  console.log(`Approved ATR IDs for activity ${activityCode}:`, approvedAtrIds);

  if (approvedAtrIds.length === 0) return { approvedCount: 0, approvedLocationIds: [] };
  ...
  /**
   * @TODO Potential optimization: instead of looping through tables and running separate UPDATEs,
   * we could build one giant UPDATE with a big OR condition that covers all tables at once.
   * This would reduce the number of queries and round-trips, but it would be more complex to
   * construct and maintain — especially with the different table schemas and status column presence.
   */
  ...
  const noStatusTables = allDataTables.filter((t) => !GHG_TABLES_WITH_STATUS.has(t));
  ...
  for (const [parentTable, children] of Object.entries(CHILD_TABLES_BY_PARENT)) {
    ...
    if (!(GHG_TABLES_BY_ACTIVITY[activityCode] ?? []).includes(parentTable)) continue;
