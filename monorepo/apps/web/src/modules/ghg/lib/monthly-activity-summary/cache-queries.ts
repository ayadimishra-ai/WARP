...
type ChildConfig = { kind: "child"; table: string; parentTable: string; parentFk: string };
...
function buildRecountSQL(
  ...
  for (const cfg of configs) {
    if (cfg.kind === "direct") {
      const statusExpr = cfg.hasStatus ? "COALESCE(gd.status, atr.status)" : "atr.status";
      branches.push(`
...
export async function upsertCacheForActivity(params: CacheUpsertParams): Promise<void> {
  ...
  logger.info(`[Cache Upsert] ${activityCode} @ ${organizationAddressId}: ${monthYears.length} month-year pairs processed.`);
}
...
export function expandToCalendarMonthYears(
  ...
  return targets.map((m) => {
    ...
    const calYear = startMonth === 1 || idx < 0 || idx >= startMonth - 1 ? fyYear : fyYear + 1;
...
export async function queryTaskRequestSummaryFromCache(params: SummaryQueryParams): Promise<any[]> {
  const {
    organizationId,
  ...
  const validCodes = (activityCodes ?? []).filter((c) => /^[a-zA-Z0-9_]+$/.test(c));
  ...
  const statusWhereClause = statusFilter === "pending"
    ? "WHERE pending > 0"
