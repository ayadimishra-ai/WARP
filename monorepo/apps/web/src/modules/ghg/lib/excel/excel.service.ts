...
import { sanitize_compare_str_v2 } from "@/modules/ghg/utils/comapre.util";
...
// Normalize decimal values to preserve precision from Excel
...
function normalizeObject(obj: any): any {
  ...
  if (obj && typeof obj === "object") {
    return Object.fromEntries(
      Object.entries(obj).map(([k, v]) => [k, normalizeObject(v)])
    );
  }
  ...
  await Promise.all(
    workbook.SheetNames.map(async (sheetName) => {
      ...
      if (datarange) {
        ...
        // Normalize decimal values to preserve exact precision from Excel
        const cleanedData = rows.map((row) => normalizeObject(row));
        data.push({ sheetName, data: cleanedData });
      } else {
