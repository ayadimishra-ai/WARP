...
import { GetGhgMaterialProcurementDataByTaskRequestIdsQuery } from "@/modules/ghg/graphql/queries/get-ghg-material-procurement-data-by-task-request-ids.generated";
...
import { getMonthNumberAndIndex } from "@/modules/ghg/utils/date.util";
...
const fetchBuyerSupplierMappingsInBatches = async (
  sdk: Awaited<ReturnType<typeof getGraphQlServerSDK>>,
  whereConditions: Record<string, any>[],
  batchSize = 1000
) => {
  const uniqueWhereConditions = _.uniqWith(whereConditions, _.isEqual);
  const batches: Record<string, any>[][] = [];

  for (let i = 0; i < uniqueWhereConditions.length; i += batchSize) {
    batches.push(uniqueWhereConditions.slice(i, i + batchSize));
  }

  const results = await Promise.allSettled(
    batches.map((batch) =>
      sdk.GetBuyerSupplierAddressMappingData({ where: { _or: batch } })
    )
  );

  return results.reduce<
    NonNullable<
      Awaited<ReturnType<typeof sdk.GetBuyerSupplierAddressMappingData>>
    >["BuyerSupplierAddressMappings"]
  >((acc, result) => {
    if (result.status === "fulfilled") {
      acc.push(...(result.value.BuyerSupplierAddressMappings ?? []));
    }
    return acc;
  }, []);
};

const fetchMaterialProcurementsByMonthYearInBatches = async (
  sdk: Awaited<ReturnType<typeof getGraphQlServerSDK>>,
  whereConditions: Record<string, any>[],
  supplier_codes: string[],
  batchSize = 1000
): Promise<GetMaterialProcurementsByMonthYearOrgAddressIdsQuery> => {
  const uniqueWhereConditions = _.uniqWith(whereConditions, _.isEqual);
  const batches: Record<string, any>[][] = [];
  ...
  const results = await Promise.allSettled(
    batches.map((batch) =>
      sdk.GetMaterialProcurementsByMonthYearOrgAddressIds({
        whereCondition: { _or: batch },
        supplier_codes,
      })
    )
  );

  const taskRequests = results.reduce<
    NonNullable<
      GetMaterialProcurementsByMonthYearOrgAddressIdsQuery["TaskRequest"]
    >
  >((acc, result) => {
    if (result.status === "fulfilled") {
      acc.push(...(result.value.TaskRequest ?? []));
    }
    return acc;
  }, []);

  return { TaskRequest: taskRequests };
};
...
const getEmissionFromSupplier = async ({
  ...
  try {
    ...
    const buyerOPwithSupplierMappingConfig = {
      BuyerSupplierAddressMappings: await fetchBuyerSupplierMappingsInBatches(
        sdk,
        BuyerSupplierWhereCondition
      ),
    };
    ...
    const materialDataFromOtherLocation =
      await fetchMaterialProcurementsByMonthYearInBatches(
        sdk,
        whereCondition,
        supplierCodesForOtherLoc
      );
