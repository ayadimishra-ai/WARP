...
import { getAccociatedBuyers } from "@/modules/ghg/components/activity-data-records/activity-data-records-server-action";
...
import { getMonthNumberAndIndex, months } from "@/modules/ghg/utils/date.util";
...
export const saveEmissionDashboard = async (
  ...
  for (let index = 0; index < transportdata.length; index++) {
    kpiemissionbytransportation.push({
      ...
      metadata: {},
      kpi_em_AllFuels_Consumption: transportdata[index]?.kpi_em_AllFuels_Consumption,
    });
  }
