...
import { checkConflictsForCapitalGoods } from "../cross-template-validation";
...
import type { IUoMValidationWarning, IBulkUploadValidationResult } from "@/modules/ghg/lib/uom-category/uom-validation-warning.types";
      ...
      const isDifferentUoMGroupForSameMaterial = sheet?.data?.some(
        (item, itemIndex) => {
          ...
          const itemUoMMaster = uoMMasterArray?.find(
            (master) => sanitizeString.v4(String(master?.value || "")) === itemUoM
          );
