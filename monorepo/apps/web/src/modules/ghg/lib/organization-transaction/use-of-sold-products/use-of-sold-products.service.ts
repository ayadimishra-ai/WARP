...
import {
  stripScriptContent,
  stripScriptContentOrNull,
} from "@/modules/ghg/utils/script-stripper/script-stripper.util";
 ...
 * Each value is run through stripScriptContent to remove HTML/script payloads.
 */
const buildWorkingDetailsMetadata = (row: Record<string, any>) => {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (row[key] !== undefined && row[key] !== null && row[key] !== "") {
      const cleaned = stripScriptContent(row[key]);
    ...
    if (activityTaskData.length > 0) {
      ...
      if (
        ...
        sheetRecord.push({
          ...
          Type_of_Fuel_Consumed: stripScriptContent(
            row["Type of Fuel Consumed"]
          ),
          Product_Code: stripScriptContent(row["Product Code"]),
          Lifetime_of_Product: stripScriptContentOrNull(
            row["Lifetime of Product"]
          ),
          Rationale: stripScriptContentOrNull(row["Rationale"]),
          Quantity_of_Fuel_Consumed: Number(
            row["Quantity of Fuel Consumed (product lifetime)"]
          ),
          UoM_of_Fuel_Consumed: stripScriptContent(row["UoM of Fuel Consumed"]),
          Additional_comments: stripScriptContentOrNull(
            row["Additional comments"]
          ),
          Remarks: stripScriptContentOrNull(row["Remarks"]),
        ...
        updateData.push({
          ...
          _set: {
            ...
            UoM_of_Fuel_Consumed: stripScriptContent(
              row["UoM of Fuel Consumed"]
            ),
            Additional_comments: stripScriptContentOrNull(
              row["Additional comments"]
            ),
    ...
    if (activityTaskData.length > 0) {
      ...
      if (
        ...
        sheetRecord.push({
          ...
          Rationale: stripScriptContentOrNull(row["Rationale"]),
          Region: stripScriptContent(row["Region"]),
    ...
    if (activityTaskData.length > 0) {
      ...
      if (
        ...
        sheetRecord.push({
          ...
          Rationale: stripScriptContentOrNull(row["Rationale"]),
          Refrigerant_type_used_in_sold_product: stripScriptContent(
            row["Refrigerant type used in sold product"]
          ),
          Quantity_of_Refrigerant_consumed: Number(
            row["Quantity of Refrigerant consumed"]
          ),
          UoM_of_Refrigerant_consumed: stripScriptContent(
            row["UoM of Refrigerant consumed"]
          ),
          Additional_comments: stripScriptContentOrNull(
            row["Additional comments"]
          ),
