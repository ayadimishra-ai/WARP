...
import { checkConflictsForUpstreamOrMaterialProcurement } from "@/modules/ghg/lib/cross-template-validation";
import {
  getUoMCategory,
  getMaterialMasterUoMCategory,
  validateUoMCategoryCompatibility,
} from "@/modules/ghg/lib/uom-category";
import type { IUoMValidationWarning } from "@/modules/ghg/lib/uom-category/uom-validation-warning.types";
    ...
    sheet.data.forEach(async (dataItem: Record<string, string>) => {
      ...
      if (
        ...
        if (materialHasWeight) {
          ...
          const masterUoMValue = matchingMaster?.Material_Quantity_Procured_uom || "";
...
const validateDatabyDb = async (
  ...
  let materialWeightData: { Material_Code?: string | null; Material_Weight_Per_Unit?: any | null; Material_Quantity_Procured_uom?: string | null }[] = [];
  if (materialCodes.length > 0) {
    const escapeLike = (s: string) => s.replace(/[%_\\]/g, (m) => "\\" + m);
    const toIlikePattern = (s: string) => {
  ...
  excelData.forEach((dataItem) => {
    ...
    if (dataItem.sheetName === "Upstream - Road") {
      dataItem.data.map((item: any) => {
        ...
        // Check if this material has weight configured in Material Master
        const materialKeyNormalized = sanitizeString.v4(String(materialCode || ""));
        const materialHasWeight = materialWeightData?.some(
          (master) =>
            master?.Material_Code &&
            sanitize_compare_str_v4(String(master?.Material_Code), materialKeyNormalized) &&
            master?.Material_Weight_Per_Unit !== 0 &&
            master?.Material_Weight_Per_Unit !== null &&
            !!master?.Material_Quantity_Procured_uom
        );
        const masterDataUoMValue = materialWeightData?.find(
          (master) =>
            master?.Material_Code &&
            sanitize_compare_str_v4(String(master?.Material_Code), materialKeyNormalized)
        )?.Material_Quantity_Procured_uom;

        const denominatorUom = [...(masterDataUoMValue ?? [])][0]?.split("/")?.[1] ?? "";
        const masterDataCategory = getUoMCategory(
          activityMasterData.ActivityMaster || [],
          TRANSPORT_UPSTREAM_MATERIAL_QUANTITY_PROCURED_UOM_KEY,
        ...
        if (materialHasWeight) {
          ...
          const differentGroupOnOtherSheet = uploadedData?.some(
            (x) => {
              if (x.sheetName !== otherSheet) return false;
              if (!sanitize_compare_str_v4(x.materialCode, materialCode)) return false;
...
const validateUOMConsistencyByUoMGroup = (
  ...
  const denominatorUom = [...(existingMaterialMasterUOMs ?? [])][0]?.split("/")?.[1] ?? "";
  const masterDataCategory = getUoMCategory(
    activityMasterData,
    TRANSPORT_UPSTREAM_MATERIAL_QUANTITY_PROCURED_UOM_KEY,
    String(denominatorUom)
  ...
  for (const uom of existingMaterialMasterUOMs) {
    ...
    if (existingDenominatorGroup && existingDenominatorGroup !== currentMaterialUomGroup) {
      const existingCategory = existingDenominatorGroup as import("@/modules/ghg/lib/uom-category").UoMCategory | null;
...
const validateTransportUpstreamUOMConsistencyDataSheet = async (
  sheet: TExcelSheet,
  organizationId: UUID
): Promise<{ errors: Record<string, string>[]; warnings: IUoMValidationWarning[] }> => {
  const sheetAllErrorEntries: Record<string, string>[] = [];
...
const validateSheetUOMConsistencyMethods: Record<
  TTransportUpstreamActivitySheetNames,
  (sheet: TExcelSheet, organizationId: UUID) => Promise<{ errors: Record<string, any>[]; warnings: IUoMValidationWarning[] }>
> = {
...
const validateUOMConsistency = async (
  excelData: TExcelSheet[],
  organizationId: UUID
): Promise<{ errorSheets: TExcelSheet[]; warnings: IUoMValidationWarning[] }> => {
  const excelSheetData: TExcelSheet[] = [];
  ...
  const uniqueMaterialCodes = Array.from(
    new Set(
      excelData.flatMap((sheet) =>
        sheet.data
          .map(
            (item: Record<string, string>) => item["Material Procured Code"]
          )
