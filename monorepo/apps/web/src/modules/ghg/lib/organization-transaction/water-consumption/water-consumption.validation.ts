...
export const validateduplidateDataByKey = (
  ...
  const quantityColumnsBySheet: Record<string, string[]> = {
    "Freshwater Use": [
      "Total Fresh Water Used for Domestic Use",
      "Total Fresh Water Used for Industrial Use",
      "Total Fresh Water Used for Landscaping",
      "Total Fresh Water Used for Miscellaneous Uses",
    ],
    "Wastewater Reuse": [
      "Total Treated Effluent Reused for Domestic Use",
      "Total Treated Effluent Reused for Industrial Use",
      "Total Treated Effluent Reused for Landscaping",
      "Total Treated Effluent Used for Miscellaneous Uses",
    ],
    "Harvested Water Use": [
      "Total Harvested Water Used for Domestic Use",
      "Total Harvested Water Used for Industrial Use",
      "Total Harvested Water Used for Landscaping",
      "Total Harvested Water Used for Miscellaneous Uses",
    ],
  };
  ...
  const uniqueKeyindex = `${Year}-${month}-${sheet_uom}`;
  ...
  if (length > 0) {
    ...
    const currentRow = sheet.data[index - 1] ?? {};
    const quantityColumns = quantityColumnsBySheet[sheetName] ?? [];
    const allQuantitiesBlank =
      quantityColumns.length > 0 &&
      quantityColumns.every((column) => {
        const value = currentRow[column];
        return value === undefined || value === null || String(value).trim() === "";
      });

    if (allQuantitiesBlank) {
      quantityColumns.forEach((column) => {
        errorEntries.push({
          column,
          row: index,
          errorMessage:
            "Please add numerical values for the quantity for this year and month and uom",
        });
      });
    }
  }
