...
export const generateFormFields_new = (
  ...
  Object.values(sectionData).forEach((section, index) => {
    ...
    questions
      ...
      .forEach((question) => {
        const mainQuestionId = determineMainQuestionId(questions, question.id);
...
export const processQuestionsForSection = (
  ...
  if (question.isMainQuestion) {
    ...
    if (!!displayOption && displayOption.trim() !== "") {
      formatValue_date = {
        orientation: displayOption,
      };
    }
    ...
    if (question.type?.trim().toLowerCase() === "date") {
      formatValue_date = {
        format: format || null,
        orientation: formatValue_date.orientation,
      };
    }
    ...
    const containerFormField: FormField_Insert_Input = {
      ...
      questionId: MainQuestionId || null, // Only use the actual question ID
    ...
    const interfaceOptionsWithFirstQuestion = {
      ...interfaceOptions,
    ...
    const questionFormField: FormField_Insert_Input = {
      ...
      subtheme: "",
      interfaceOptions: interfaceOptionsWithFirstQuestion,
      fieldOptions: {
        label: label,
        enable:
    ...
    console.log(`Child question ${question.questionCode} details:`, {
      ...
      interfaceType,
      fieldType,
      parentQuestionId: MainQuestionId || "N/A",
...
const determineMainQuestionId = (
  ...
  const mainQuestionId = questions?.find(
    (q: QuestionToInsert) => q.id === childQuestionId,
  );

  if (!!mainQuestionId && !!mainQuestionId?.parentQuestionId) {
    return determineMainQuestionId(
      questions,
      mainQuestionId?.parentQuestionId,
      visited,
    );
  }

  console.log("mainQuestionId", mainQuestionId);
  console.log("childQuestionId", childQuestionId);
  return mainQuestionId?.id || childQuestionId;
};
...
const determineInterfaceAndType = (
  ...
  let MainQuestionId: string | null = null;
  ...
  const parentQuestionId = question.parentQuestionId || null;

  if (question.isMainQuestion) {
    MainQuestionId = question.id;
  }

  let decimalLimit: number | null = null;
  let SelectAllChoice: string | null = null;
  let SelectNoneChoice: string | null = null;
  let displayOption: string | undefined = undefined;
  let isAutoCalculate: boolean = false;
  let format: string = question.format || "";
  let decimalKey = "";
  let fileFormat: string[] = [];
  let minValue = question.min || null;
  let maxValue = question.max || null;
  let maxColumns: number = 3;
  let minRows: number = 1;
  ...
  question.format?.split(";").forEach((value: string) => {
    ...
    if (key === "format") {
      if (type.includes("file")) {
        val.split(",").forEach((value: string) => {
          ...
          switch (key) {
            ...
            case "png":
              ...
              break;
            case "gif":
              fileFormat.push("image/gif");
              break;
            case "webp":
              fileFormat.push("image/webp");
              break;
            case "svg":
              fileFormat.push("image/svg+xml");
              break;
            case "mp4":
              fileFormat.push("video/mp4");
              break;
            case "webm":
              fileFormat.push("video/webm");
              break;
            case "doc":
              fileFormat.push(
            ...
            case "docx":
              ...
              break;
            case "csv":
              fileFormat.push("text/csv");
              fileFormat.push("text/comma-separated-values");
              fileFormat.push("text/tab-separated-values");
              fileFormat.push("text/plain");
              fileFormat.push("application/csv");
              break;
            case "xls":
              fileFormat.push(
            ...
            case "zip":
              ...
              break;
            case "rar":
              fileFormat.push("application/x-rar-compressed");
              break;
            case "7z":
              fileFormat.push("application/x-7z-compressed");
              break;
            default:
              break;
          }
    ...
    if (key === "decimal") {
      decimalLimit = val.includes(".") ? val.split(".")[1].length : null;
      decimalKey = key;
    }

    if (key === "selectallchoice") {
      SelectAllChoice = val || "";
    }
    if (key === "selectnonechoice") {
      SelectNoneChoice = val || "";
    }

    if (key === "display") {
      displayOption = val || "horizontal";
    }

    if (key === "maxcolumns") {
      maxColumns = Number(val) || 3;
    }
    if (key === "minrows") {
      minRows = Number(val) || 1;
    }
  });
  ...
  question.customValidation?.split(";").forEach((value: string) => {
    ...
    if (key === "autocalculate") {
      isAutoCalculate = val.toLowerCase() === "true";
    }
    if (key === "rule") {
      rule = val.trim().replace(/\./g, "_") || "";
      rule = rule.replace(/\_value/g, ".value");
    }
    if (key === "message") {
      message = val || "";
    }
  ...
  if (visibleCondition && visibleCondition.trim() !== "") {
    ...
    let visibleConditionField =
      visibleCondition?.trim().replace(/\./g, "_") || "";
    visibleConditionField = visibleConditionField.replace(/_value/g, ".value");
    visibleConditionRules = [
      {
        name: "if required",
        rule: visibleConditionField,
        fieldOptions: {
          enable: true,
        },
      },
  ...
  if (type.includes("text")) {
    ...
    if (decimalKey === "decimal") {
      ...
      interfaceOptions = {
        ...
        maxValue: maxValue ? maxValue : 999999999999999,
      };
    } else {
    ...
    interfaceOptions = {
      ...
      accept: fileFormat,
      showLabel: false,
      ...(maxValue ? { maxSizeInMb: maxValue } : { maxSizeInMb: 2 }),
    ...
    interfaceOptions = {
      ...
      creatable: false,
      searchable: true,
      ...(SelectAllChoice && typeof SelectAllChoice !== "undefined"
        ? { selectAllChoice: SelectAllChoice }
        : {}),
      ...(SelectNoneChoice && typeof SelectNoneChoice !== "undefined"
  ...
  } else if (type.includes("multiline input")) {
