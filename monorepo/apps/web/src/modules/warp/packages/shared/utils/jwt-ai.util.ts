import type { GetActiveSubscriptionByCompanyIdQuery } from "@/modules/warp/packages/graphql/generated/types";
...
export const canEnableOPSToIQCuration = async (
  ...
  try {
    ...
    const response = await fetch(
      `/warp/api/v1/ops/check-company-eligibility?companyId=${encodeURIComponent(companyId)}`,
      {
        method: 'GET',
      }
    );
