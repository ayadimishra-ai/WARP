...
import { IconClockPause, IconFilter, IconInfoCircle } from "@tabler/icons-react";
import InvitationButtons from "@/modules/warp/packages/client/features/invitation/components/InvitationButtons";
import {
  DataTableType,
  PaginationType,
  StatesFilterType,
  TableHeader,
  useAssessment,
} from "@/modules/warp/packages/client/hooks/use-assessment";
import Spinner from "@/modules/warp/packages/client/layouts/Spinner";
import { GetAssessmentListingDetailsQuery } from "@/modules/warp/packages/graphql/generated/types";
...
import { useGetGlobalMasterByInternalRequestCompanyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-global-master-by-InternalRequestCompany";
import { useGetGlobalMasterDataForInviterFormAutoAppoverQuery } from "@/modules/warp/packages/graphql/queries/generated/get-global-master-data-for-inviter-form-auto-appover";
...
const TH: FC<THPropsType> = ({
  ...
  const showProcessing = isUserAI || (statesfilter?.hasOPSToIQCuration?.() ?? false);
  const statuses = [
    "All",
    ...
    ...(showProcessing ? [FormInvitationUIStatus.Processing] : []),
  ...
  return (
    ...
    >
      <Group pos="relative" w="max-content" gap={0}>
        <UnstyledButton
        ...
        {title === "Questionnaire" && (
          <Box>
            ...
            {opened && (
              <Paper pos="absolute" bottom="-55px" w="300px" p="8px" left="0" ref={ref} shadow="sm">
                <Box className={`${classes.innersrchbox} searchBoxInner`}>
                  <Autocomplete
                    classNames={{ dropdown: `${classes.dropdownWidth} mantine-Autocomplete-dropdown` }}
                    w="100%"
                    ...
                    onOptionSubmit={(_optionValue: string) => { const e = { value: _optionValue };
        ...
        {title === "Status" && (
          ...
          >
            ...
            <HoverCard.Dropdown className={classes.dropdownHoverCard}>
              <Flex direction="column" align="self-start" gap="4px">
                ...
                (
                  ...
                  >
                    <td className={classes.tdStyle}>
                      ...
                      {/* <Text fz={12} pl={15} title={row.original.Questionnaire}>
                    {row.original.Questionnaire}
                  </Text> */}
                    </td>
                    {/* {
                  ...
                  >
                    ...
                    {chkCompanyId !== undefined &&
                      ...
                      locationCount.Count > 1 && (
                        ...
                        <td className={classes.tdStyle}>
                          <Text fz={12} title={row.original["Location"]}>
                    ...
                    <td className={classes.tdStyle}>
                      <Text fz={12} title={row.original.Period}>
                        {row.original.Period}
                      </Text>
                    </td>
                    <td className={classes.tdStyle}>
                      <Text fz={12} title={row.original["Requested On"]}>
                    ...
                    <td className={classes.tdStyle}>
                      <Box className={classes.statusTd}>
                        {row.original.Status.toLowerCase() ===
                            ...
                            <Button
                              color="aiGradientBtn"
                              leftSection={<AISparkleIcon width={14} height={14} />}
                    ...
                    {userSession?.user?.role !== AppRoles?.Responder && (
                      <td className={classes.tdStyle}>
                        ...
                        row.original.Submission_Status === "InProgress" ? (
                          ...
                          <Text fz={12}>{row.original.Score}</Text>
                        ) : (
                          <Text fz={12}>
...
const StatesHeader = ({
  ...
  return (
    ...
    //
    <>
      <Flex mb={15} mt={0} maw="45%" gap="0 15px" className={[classes.topTableFilters, classes.commonMargin].join(" ")}>
        {statuses.map((element, index: number) => (
          <>
      ...
      <Box className={classes.textFieldFilter}>
        ...
        <Flex className={classes.innersrchbox2} align="center" gap={16}>
          {locationCount.IsSortButtonShow > 0 && (
            <Flex gap={5}>
              <UnstyledButton onClick={SortByComment}
              >
                <Text
                  fz={16}
          ...
          {userRole === "Inviter" && (
            <Flex gap={5}>
              <UnstyledButton onClick={listingPageDataInExcel}
              >
                <Text
                  fz={16}
                  className={classes.filterHover}
          ...
          {isShowClearAll === true && (
            <UnstyledButton onClick={clearAllData}>
              <Text
                fz={16}
                className={classes.filterHover}
                c={"#444"}
