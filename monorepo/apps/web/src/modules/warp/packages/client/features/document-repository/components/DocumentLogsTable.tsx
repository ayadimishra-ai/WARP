...
import PaginationFooter from "@/modules/warp/packages/client/components/PaginationFooter";
import SortIcons from "@/modules/warp/packages/client/components/SortIcons";
import SearchIcon from "@/modules/warp/packages/client/components/svgIcons/SearchIcon";
import Spinner from "@/modules/warp/packages/client/layouts/Spinner";
import { useGetDocumentLogsWithAiSuggestedDocumentsQuery } from "@/modules/warp/packages/graphql/queries/generated/get-document-logs-with-ai-suggested-documents";
...
const TH: FC<THPropsType> = ({
  ...
  return (
    <th className={classes.tableHeadingBorder}>
      <Group style={{ position: "relative", width: "max-content" }} gap={0}>
        <UnstyledButton
        ...
        {field !== "status" &&
          ["cardName", "fileName", "uploadedBy"].includes(field || "") && (
            <Box>
              ...
              {opened && (
                <Paper pos="absolute" bottom={-55} left={0} w={300} p={8}
                  ref={ref}
                  shadow="sm"
                >
                  <Box className={`${classes.innersrchbox} searchBoxInner`}>
                    <Autocomplete
                      ...
                      onOptionSubmit={(_optionValue: string) => {
                        const e = { value: _optionValue };
                        if (onFilter && field) {
          ...
          paginatedData.map((item, index) => (
            <tr key={index} className={classes.trStyle}>
              <td className={classes.tdStyle}>
                <TextWithTooltip fz={12} text={item.cardName}>
                  {item.cardName}
                </TextWithTooltip>
              </td>
              <td className={classes.tdStyle}>
                {(() => {
                  ...
                  return originalItem && originalItem.fileUrl ? (
                    ...
                    <TextWithTooltip fz={12} w="max-content" text={item.fileName}>
                      {item.fileName}
                    </TextWithTooltip>
                  );
                })()}
              ...
              <td className={classes.tdStyle}>
                <TextWithTooltip fz={12} w="max-content" text={item.uploadedBy}>
                  {item.uploadedBy}
                </TextWithTooltip>
              </td>
              <td className={classes.tdStyle}>{item.deletedOn}</td>
              <td className={classes.tdStyle}>
                <TextWithTooltip fz={12} w="max-content" text={item.deletedBy}>
                  {item.deletedBy}
                </TextWithTooltip>
              </td>
              <td className={classes.tdStyle}>
    ...
    <>
      ...
      >
        ...
        <Box className={classes.textFieldFilter}>
          <Flex align="center" gap={16} justify="flex-end" wrap="wrap">
            {isShowClearAll === true && (
...
const DocumentLogsTable: FC = ({}) => {
  ...
  if (error) return <Text c="red">Error loading document logs</Text>;
