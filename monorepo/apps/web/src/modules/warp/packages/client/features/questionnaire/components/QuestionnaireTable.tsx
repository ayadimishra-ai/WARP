...
const TH: React.FC<THPropsType> = ({
  ...
  return (
    <th className={classes.tableHeadingBorder} style={{ width: "200px" }}>
      ...
      <td className={classes.tdStyle}>
        <Group gap="xs" className={classes.actionGroup}>
          <Tooltip
            position="bottom"
            label={
              questionnaire.isOldQuestionnaire
                ? "Upload disabled for old questionnaires"
                : "Upload Excel File"
            }
          >
            <ActionIcon
              variant="transparent"
              onClick={() =>
                !questionnaire.isOldQuestionnaire && onUpload(questionnaire)
              }
              disabled={questionnaire.isOldQuestionnaire}
              style={{
                cursor: questionnaire.isOldQuestionnaire
                  ? "not-allowed"
                  : "pointer",
                opacity: questionnaire.isOldQuestionnaire ? 0.5 : 1,
              }}
            >
              <UploadIcon
                color={questionnaire.isOldQuestionnaire ? "#999" : "#003b52"}
              />
            </ActionIcon>
          </Tooltip>
          ...
          <Tooltip position="bottom" label="Assign questionnaires">
            <ActionIcon
              variant="transparent"
          ...
          <Tooltip position="bottom" label="Preview">
            <ActionIcon
              disabled={
                questionnaire.FormInvitationId?.length === 0 ||
                questionnaire.FormInvitationId?.[0] === undefined
              }
              onClick={() => {
          ...
          <Tooltip
            position="bottom"
            label={
              !questionnaire.logsCount || questionnaire.logsCount === 0
          ...
          >
            <ActionIcon
              variant="transparent"
              onClick={() => {
                if (questionnaire.logsCount && questionnaire.logsCount > 0) {
                  window.location.href = `/warp/QuestionnaireLogsPage?formid=${questionnaire.id}`;
                }
              }}
              style={{
                cursor:
