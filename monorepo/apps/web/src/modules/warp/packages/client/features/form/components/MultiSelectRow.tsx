...
const MultiSelectRow: FormFieldControl<"multi-select-row"> = ({
  ...
  // Derive labels and field-names from the already-memoized sortedChildren.
  // Using useMemo here means typing in any cell doesn't rebuild these arrays
  // (which the CommonTable header used to receive as a fresh array every render).
  const sortedChildrenLabels = useMemo(
  ...
  if (
    ...
    recommedationData = getJsonataExpression(
    ...
    >
      ...
      {hasAdditionalOptions && !!answerRef.current?.length && (
        <Box style={{ width: "100%" }}>
          <CommonTable
            headers={[
              <Text key={"action" + formField.id}>Action</Text>, // This is for the Delete button column
...
SimpleMultiSelectRow.displayName = "SimpleMultiSelectRow";
...
SelectMultiSelectRowWrapper.displayName = "SelectMultiSelectRowWrapper";
