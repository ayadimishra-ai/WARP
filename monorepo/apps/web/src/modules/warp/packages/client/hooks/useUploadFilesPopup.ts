import { useFileUploadWithProgress } from "@/modules/warp/packages/client/hooks/use-file-upload-with-progress";
...
import { useBulk_Insert_Document_LogFilesMutation } from "@/modules/warp/packages/graphql/mutations/generated/bulk-insert-document-logs-files";
...
import { useGetAiSuggestedDocumentsQuery } from "@/modules/warp/packages/graphql/queries/generated/get-AI-suggested-documents";
...
import { useGetFormInvitationDetailsbyIdLazyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-form-invitation-details-by-id";
import {
...
export const useUploadFilesPopup = (): UseUploadFilesPopupReturn => {
  ...
  // App Router replacement for Pages-Router `useRouter().query`. The legacy
  // hook used `const { query } = useRouter()` and references like
  // `query?.invitationId` survive below. Without this shim those references
  // throw ReferenceError because `query` is undeclared, which silently breaks
  // handleSubmitSelectedDocuments and prevents the file input from updating.
  const query = useMemo(() => {
  ...
  const allowMultiple = searchParams?.get("allowMultiple") === "true";
  ...
  const {
    ...
    // Don't fire the query until the user session resolves and we have a
    // companyId. Without this, Apollo runs once with companyId=undefined,
    // returns an empty list, and the popup briefly renders the
    // "No documents in your repository" empty state before the session
    // loads and the query refetches.
    skip: !companyId,
  });
  ...
  // Tracks the async post-fetch document grouping below. The popup's loading
  // flag combines this with Apollo's `documentLogsLoading` so the table never
  // flashes "No documents in your repository" while the data is still being
  // assembled.
  const [isProcessingDocuments, setIsProcessingDocuments] = useState<boolean>(true);
  ...
  useEffect(() => {
    ...
    const processDocuments = async () => {
      if (!documentLogsData?.DocumentLogs) {
        // Apollo hasn't returned yet (or query is skipped because companyId is
        // not ready). Keep `isProcessingDocuments` true so the parent stays in
        // its loading state until real data arrives.
  ...
  // Combined loading flag exposed to the popup. We're "loading" while:
  //  - the user session hasn't resolved yet (so companyId is undefined and
  //    the docs query is skipped),
  //  - Apollo is fetching, or
  //  - the post-fetch processDocuments effect is still running.
  const loading = !companyId || documentLogsLoading || isProcessingDocuments;
