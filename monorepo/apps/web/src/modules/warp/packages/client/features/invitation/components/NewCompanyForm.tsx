...
import { useInsertCompanyMutation } from "@/modules/warp/packages/graphql/mutations/generated/insert-company";
import { useInsertcompanyformfundtypeMutation } from "@/modules/warp/packages/graphql/mutations/generated/insert-company-form-fundtype";
...
import { useInsertInvitationConsultantMappingMutation } from "@/modules/warp/packages/graphql/mutations/generated/insert-invitation-consultant-mapping";
import { useUpdateCompanyDetailsMutation } from "@/modules/warp/packages/graphql/mutations/generated/update-company-manufacturing";
import { useUpdateParentCompanyMappingbyCompanyIdMutation } from "@/modules/warp/packages/graphql/mutations/generated/update-parentcompanymapping-by-companyId";
import { useGetAssessorConsultantMappingQuery } from "@/modules/warp/packages/graphql/queries/generated/get-assessor-consultant-mapping";
import { useGetCompanyByParentCompanyIdNullQuery } from "@/modules/warp/packages/graphql/queries/generated/get-company-by-parent-company-id-null";
import { useGetcompanyfundtypeavailabilityQuery } from "@/modules/warp/packages/graphql/queries/generated/get-company-fundtype-availability";
import { useGetCompanyDetailByIdQuery } from "@/modules/warp/packages/graphql/queries/generated/get-companydetail-by-id";
import { useGetFormInvitationByFormIdLazyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-form-invitation-by-form-id";
...
import { useGetGlobalMasterByCountryQuery } from "@/modules/warp/packages/graphql/queries/generated/get-global-master-by-country";
import { useGetGlobalMasterByTypeQuery } from "@/modules/warp/packages/graphql/queries/generated/get-global-master-by-type";
import { useGetglobalmasterdataforfundtypeQuery } from "@/modules/warp/packages/graphql/queries/generated/get-globalmaster-data-for-fundtype";
...
import InvitationNewCompanySchema from "@/modules/warp/packages/shared/validation/invitation-new-company.schema";
...
const NewCompanyFormFields = () => {
  ...
  const {
    ...
    resolver: yupResolver(InvitationNewCompanySchema) as any,
  });
    ...
    if (!!invitedStatusInvitation && invitedStatusInvitation.length > 0) {
      ...
      await Promise.all(
        invitedStatusInvitation
          // .filter((items) => items.isNormalInvitation)
          .map(async (items) => {
            await fetch("/warp/api/email-invitation", {
              method: "POST",
              headers: {
                "content-type": "application/json",
              },
    ...
    try {
      ...
      for (let rec of data.companyForm) {
        if (rec.companyId === "") {
          ...
          await fetch(
            `/warp/api/get-company-by-name-and-primary-contact?companyName=${rec.companyName}&primaryContactEmail=${rec.primaryContactEmail}&primaryContactPhone=${rec.primaryContactMobileNumber}`,
            {
              method: "GET",
              headers: {
                "content-type": "application/json",
  ...
  const onSubmit = async (e: any) => {
    ...
    captchaValidationBeforeSubmitHandler(async () => {
      ...
      if (!isValidAllFields || !isFundTypeValid) {
        setIsSubmitting(false);
        return;
      }
  ...
  const renderMessage = () => {
    if (recomNewAssessmentPeriod) {
      return "Please select the historical months only for assessment period.";
    }
    if (assessmentRequired) {
      return "Please select an Assessment";
    }
    if (assessmentPeriodRequired) {
      return "Assessment period selection is mandatory";
    }
    if (assessmentPeriodError) {
      return "Assessment period selected is invalid. Please select valid period.";
    }
    if (existingCompanyName) {
      return `Assessment company details for ${CompanyName} already exists.`;
    }
    if (duplicateCompanyName) {
      return `Assessment company ${duplicateCompanyName} is duplicate.`;
    }
    return null;
  };
              ...
              <Flex justify="space-between" align="baseline">
                ...
                <Flex align="center">
                  {!errors.companyForm && index === 0 && (
                    <Group mb={0} justify="flex-end" gap={2}>
                      <Text
                        fz={12}
                        c="#444444"
                      >
                      ...
                      <ActionIcon color="#003B52" className="ActionIconStyles" variant="transparent">
                        <IconPlus
                  ...
                  {index > 0 && (
                    <Group mb={0} justify="flex-end" style={{ gap: 0 }}>
                      <Text
                        fz={12}
                        c="#444444"
                        mr={2}
