...
import {
  ...
  TableHeader,
  useRecommendation,
} from "@/modules/warp/packages/client/hooks/use-recommendation";
      ...
      >
        ...
        {title === "Status" && (
          <HoverCard width={200} position="bottom" shadow="md">
            <HoverCard.Target>
              <ActionIcon className={`${classes.filtericond} filter-icon-d`} variant="transparent">
                <IconFilter
                  size={18}
                  color="#fff"
                />
          ...
          {datatable.rows.length > 0 ? (
            datatable.rows.map((row, index) => (
              ...
              >
                ...
                <td className={classes.tdStyle}>
                  <Text fz={12} title={row.original["Raised On"]}>
                ...
                <td className={classes.tdStyle}>
                  <Text fz={12} title={row.original["Implemented On"]}>
    ...
    <Stack style={{ width: "100%", gap: 0 }}>
      ...
      >
        ...
        <Box className={classes.textFieldFilter}>
          <Flex align="center" gap={24} mb={15} className={classes.innersrchbox}>
            {!!score && score !== "NA" ? (
              <UnstyledButton
                onClick={() =>
                  onProgressReport(
