import { Divider, Flex, Stack, Text, TextInput, Tooltip } from "@mantine/core";
import { IconPercentage } from "@tabler/icons-react";
import {
  checkIfNumZeroOrMoreThan10Quad,
  numberToWordsUSD,
} from "@/modules/warp/packages/client/hooks/use-number-to-word";
import { useUserSession } from "@/modules/warp/packages/client/hooks/use-user-session";
import { useGetcompanyidbyinvitationidQuery } from "@/modules/warp/packages/graphql/queries/generated/get-companyid-by-invitationid";
import { useGetWarninginterimdataByFormIdQuery } from "@/modules/warp/packages/graphql/queries/generated/get-warningmessageinterimdataByFormId";
import {
  AICArouselData,
  AppRoles,
  FormMode,
...
import { useGetUserDetailsByInvitationIdQuery } from "@/modules/warp/packages/graphql/queries/generated/get-user-details-by-invitation-id";
...
// Stable empty-object reference used by SimpleNumberInputField when auto-calc
// is disabled, so the zustand selector returns the same value on every store
// update and does not trigger a re-render for non-autocalc number inputs.
const EMPTY_ANSWER: Record<string, any> = Object.freeze({});
...
const NumberInputField: FormFieldControl = ({ formField }) => {
  ...
  let [getAutoCalValue, setAutoCalValue] = useState<any>(null);
  ...
  const checkInputData = async (str: any) => {
    ...
    if (
      ...
      try {
        ...
        try {
          // Use cached jsonata expressions — recompiling these per keystroke
          // was a measurable hot path. The compiled expression is shared across
          // all NumberInput instances via the module-level cache.
          const thresholdExpression = getJsonataExpression(
            formField?.warningRules[0]?.Threshold_Deviation_Rule,
          );
          let Threshold_Deviation = thresholdExpression ? await thresholdExpression.evaluate(checkWarningMessage) : null;
          ...
          const actualDevExpression = getJsonataExpression(
            formField?.warningRules[0]?.rule,
          );
          let actual_dev = actualDevExpression ? await actualDevExpression.evaluate(checkWarningMessage) : null;

          if (Threshold_Deviation > Number(formField?.warningRules[0]?.ratio)) {
            const warningExpression = getJsonataExpression(formField?.warningRules[0]?.rule);
            const isWarningRule = warningExpression ? Boolean(
              await warningExpression.evaluate(
                checkWarningMessage,
              ),
            ) : false;
          ...
          warningListdata = warningListdata.filter(
            ...
            warningListdata.push({
              ...
              ratio: String(Threshold_Deviation),
  ...
  useEffect(() => {
    warningRuleCheck();
  }, [value, warningmessagedata, invitationId]);
  ...
  useEffect(() => {
    ...
    // Perform auto-calculation if rules exist. Use cached jsonata expression
    // and wrap in try/catch to avoid one rule failure killing the form.
    const rule = formField?.autoCalculatedCalculation?.[0]?.rule;
    if (rule) {
      const calculateScore = async () => {
        try {
          const score = (await getJsonataExpression(rule).evaluate(answer)) || 0;
          setAutoCalValue(score);
        } catch (e) {
          console.error("Auto calculation error in NumberInput:", e);
        }
      };
      calculateScore();
    }
    ...
    formField.id, // Added formField.id as a safe dependency
  ]);
  ...
  const warningRuleCheck = async () => {
    ...
    if (
      ...
      try {
        ...
        try {
          const ruleResult = await getJsonataExpression(formField?.warningRules[0]?.rule).evaluate(
            checkWarningMessage,
          );
          if (Boolean(ruleResult) === true) {
            view_WarningMessage =
  ...
  const changeHandler = (inputValue: any, isFromAI: boolean) => {
    ...
    if (isNumberValidate === false) {
      ...
      if (inputValue === "0" && formField.type === "year") {
        setTextValue("");
      } else {
                ...
                <>
                  ...
                  >
                    <TextInput
                      ...
                      styles={{
                        input: {
                          paddingRight: formField.type === "percentage" || interfaceOptions?.suffix ? "28px" : undefined,
                        },
                      }}
                    />
                  </Tooltip>
...
const SimpleNumberInputField: FormFieldControl<"number-input"> = memo(
  ({
      ...
      if (
        ...
        const fetchRecommendation = async () => {
          try {
            ...
            console.error("Error evaluating recommendation JSONata in SimpleNumberInput:", error);
          }
        };
        fetchRecommendation();
      }
     ...
     * Only fields that actually use auto-calculation need to react to other
     * fields' answer changes. Subscribing to `store.answer` for every number
     * input meant every single number cell in the form re-rendered on every
     * keystroke anywhere — the dominant cause of tabular typing lag.
     * Gate the subscription behind `isAutoCalculate`; non-autocalc inputs read
     * a stable empty object (selector returns the same reference each call,
     * so no re-render is triggered).
     */
    const isAutoCalcEnabled = !!formField?.interfaceOptions?.isAutoCalculate;
    const answer = useFormFieldStore((store) =>
      isAutoCalcEnabled ? store.answer : EMPTY_ANSWER,
    );
    ...
    useEffect(() => {
      if (
        ...
        // Perform auto-calculation if rules exist. Use cached jsonata
        // expression to avoid recompiling the rule per keystroke.
        const rule = formField?.autoCalculatedCalculation?.[0]?.rule;
        if (rule) {
              ...
              render={(ref: any, props: any) => (
                <>
                  ...
                  {(formField.type == "currency-number" ||
                    ...
                    formField.type == "text-with-prefix" ||
                    formField.type == "percentage") && (
                    <Text fz={13} mb={0} c="#eeb806" lh="28px" w="100%">
                      {Words}
                    </Text>
                  )}
                </>
...
SimpleNumberInputField.displayName = "SimpleNumberInputField";
...
NumberInputFieldWrapper.displayName = "NumberInputFieldWrapper";
