...
} from "@/modules/warp/packages/graphql/queries/generated/get-document-logs";
...
const DocumentRepository: FC<{}> = () => {
  ...
  const DropzoneCard = dynamic(
    () =>
      import("@/modules/warp/components/embed/AIBasedSections/Common/CustomDropzoneV2"),
    {
      ssr: false,
    }
  );
              ...
              if (
                ...
                try {
                  ...
                  const raraResponse = await axios({
                    method: "POST",
                    url: "/warp/api/rara/document-rating-direct",
                    data: raraPayload,
                    headers: {
  ...
  const handleDelete = useCallback(
    async (
      ...
      // Only applies to PDFs — the parser only ingests PDFs, so calling the
      // AIProcessing endpoint for .doc/.docx/.xls/.xlsx/images/etc. is wasted
      // work. Strip query string before checking extension.
      const isPdf = (fileUrl || "")
        .toLowerCase()
        .split("?")[0]
        .endsWith(".pdf");

      if (documentLogsId && isPdf) {
        console.log(
      ...
      } else if (documentLogsId) {
        console.log(
          `[DocumentRepository] Skipping embedding deletion (non-PDF file): ${fileUrl}`
        );
      }
              ...
              {hasExpiredDocuments && (
                ...
                >
                  ...
                  {!hasViewedExpiredTab && newExpiredDocumentsCount !== 0 && (
                    ...
                    >
                      <Text c="#fff" fz="xs" lh="22px">{newExpiredDocumentsCount}</Text>
                    </Flex>
                  )}
                </Button>
              )}
