...
const SelectMultipleCheckbox: FormFieldControl = ({ formField }) => {
  ...
  const ChoicesWrapper =
    (FormHasRecommendation?.length > 0) || recommedationData.length > 0 || state.displayOptions?.orientation === "horizontal" ? Stack : Group;
  return (
      ...
      >
        <ChoicesWrapper gap="xs">
          {state.interfaceOptions?.choices?.map((choice) => {
            return (
              <Group justify="space-between" key={choice.value} w={state.displayOptions?.orientation === "horizontal" ? "auto" : "100%"}>
                <Checkbox
...
SelectMultipleCheckbox.displayName = "SelectMultipleCheckbox";
...
SimpleSelectMultipleCheckbox.displayName = "SimpleSelectMultipleCheckbox";

const SelectMultipleCheckboxWrapper: FormFieldControl<"select-multiple-checkbox"> = (props) => {
  if (props.isSimple) return <SimpleSelectMultipleCheckbox {...props} />;
  return <SelectMultipleCheckbox {...props} />;
};

SelectMultipleCheckboxWrapper.displayName = "SelectMultipleCheckboxWrapper";
