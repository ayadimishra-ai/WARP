...
import AIUploadDocumentBtn from "@/modules/warp/packages/client/features/form/components/AIUploadDocumentBtn";
import { useUserSession } from "@/modules/warp/packages/client/hooks/use-user-session";
import IntroImage from "@/modules/warp/packages/client/icons/Introimage";
...
import { useCreateFormSubmissionMutation } from "@/modules/warp/packages/graphql/mutations/generated/create-form-submission";
import { useStartNewSubmissionByInvitationIdMutation } from "@/modules/warp/packages/graphql/mutations/generated/start-new-submission-by-invitation-id";
...
import { useGetDocumentLogsWithSourcesLazyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-document-logs-with-sources";
import { useGetExistingFormInvitationLazyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-existing-form-invitation";
import { useGetFirstDeclinedQuestionIdLazyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-first-declined-question-id";
import { useGetFormIntroByInvitationIdQuery } from "@/modules/warp/packages/graphql/queries/generated/get-form-intro-by-invitationId";
...
import { useGetLatestAnsweredQuestionIdLazyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-latest-answered-question-id";
import { useGetLatestAssignedQuestionIdLazyQuery } from "@/modules/warp/packages/graphql/queries/generated/get-latest-assigned-question-id";
import { useGetWebCurationProcessAndAiBulkDocumentProcessingQuery } from "@/modules/warp/packages/graphql/queries/generated/get-web-curation-process-and-ai-bulk-document-processing";
...
} from "@/modules/warp/packages/shared/validation/send-invitation-select-existing-company.schema";
...
import { InfoData } from "@/modules/warp/components/embed/AIBasedSections/Common/InfoCard";
...
const FormIntroScreen: FC<{ invitationId: string; isEmbeded?: boolean }> = ({
  ...
  const [latestAnsweredQuestionIdQuery, { }] =
    useGetLatestAnsweredQuestionIdLazyQuery();

  const [latestAssignedQuestionIdQuery, { }] =
    useGetLatestAssignedQuestionIdLazyQuery();

  const [getFirstDeclinedQuestionIdQuery] = useGetFirstDeclinedQuestionIdLazyQuery();
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            console.log(
              `[DocumentCount] current duration: ${currentDurationFrom.toISOString().split("T")[0]
              } to ${currentDurationTo.toISOString().split("T")[0]}`,
            );
            console.log(
              `[DocumentCount] Searching for previous period report: ${previousDurationFrom.toISOString().split("T")[0]
  ...
  const { setError, handleSubmit, control, reset } =
    useForm<SendInvitationSelectExistingCompanyType>({
      resolver: yupResolver(SendInvitationSelectExistingCompanySchema) as any,
      defaultValues: {
  ...
  const datapointsStats = [
    {
      ...
      info: `The total number of data points needed to complete the ${!!formDetails ? formDetails?.framework : ""
        }  for submission.`,
      value:
  ...
  // Case 4: Responder – Re-Submitted
  const isResponderReSubmitted =
    userSession?.user?.role === AppRoles.Responder &&
    (formSubmissionStatus === "Declined" || formSubmissionStatus === "Re-Submitted") &&
    reviewSummaryCounts.reSubmitted > 0;

  // Determine the action label text shown above the button
  const reviewActionLabel = (isReviewerReSubmitted || isResponderReSubmitted)
  ...
  const startHandler = async (toPopup: boolean, docWithAI: boolean) => {
    ...
    if (
      ...
      try {
        await axios.post("/warp/api/v1/data-flow-iq", {
          invitationId,
        });
      } catch (error) {
  ...
  const reviewHandler = async (toPopup: boolean, docWithAI: boolean) => {
    ...
    if (userSession?.user?.role === AppRoles.Responder) {
      ...
      const firstResubmittedQuestionId = allMappings
        .filter((m: any) => assignedQuestionIds.includes(m.questionId) && m.currentStatus === "Re-Submitted")[0]?.questionId;
      ...
      latestAnsweredQuestionId = firstResubmittedQuestionId || firstDeclinedQuestionId || latestAnsweredQuestionId || firstQuestion;
    }
  ...
  const focusArea: Array<string> =
    formDetails?.focusArea !== undefined &&
      ...
      Array.isArray(formDetails?.focusArea)
  ...
  const focusAreaDetails: any = formDetails?.focusArea
    ...
    ?.map((item: object) => Object.keys(item)[0]);
  ...
  return (
    ...
    >
      ...
      <Flex className={classes.intro} direction={{ base: "column", md: "row" }} gap={20} pt={10} m={0}>
        <Box style={{ flex: 0.5 }} miw={1}>
          <Stack gap={0} className="AssessmentIntroDetails-inner">
            <Group
              grow
              // position="apart"
              // style={{
                  ...
                  formDetails?.focusArea !== undefined &&
                    ...
                    <Grid.Col span="auto" className={classes.algnitmtop}>
                      <Card className={classes.frameworkcard}>
                        ...
                        <Group gap="sm" fz={16} lh="18px" fw={600}>
            ...
            <Flex gap={24} align="center" wrap="wrap">
              ...
              {/* Case 2: Parent Company – Submission Declined */}
              {isParentCompanyDeclined && !isResponderReSubmitted && (
                <Box>
                  <Button
                    color="outlineBtn"
                    my="md"
