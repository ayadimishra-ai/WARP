...
import { GetinvitationcommentQuery } from "@/modules/warp/packages/graphql/generated/types";
          ...
          return session?.user?.id === item.User?.id ? (
            <Stack gap={5} align="flex-end">
              ...
              <Box className={classes.mainUserComment}>
                <Text fz={12} c="#666666" lh={1}>
                  {item?.content}
                </Text>
              </Box>
              <Box mb={10}>
                <Text fz={9} c="#666" lh={2}>
            ...
            <Stack gap={5}>
              <Group gap={5}>
                ...
                ></div>
                <Text fw={"bold"} fz={9} c="#2C9E92" lh={2}>
                  {item?.User?.Company?.name + ", " + item?.User?.name}
                </Text>
              </Group>
