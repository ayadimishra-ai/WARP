...
import AddCommentButton from "@/modules/warp/packages/client/features/form/components/AddCommentButton";
import Spinner from "@/modules/warp/packages/client/layouts/Spinner";
import { invitationFormquestionredirect } from "@/modules/warp/packages/client/services/platform-window-message.service";
...
import { useGetFormFieldsAndAnswerDataByInvitationIdQuery } from "@/modules/warp/packages/graphql/queries/generated/get-formfields-and-answer-data-by-invitationId";
        ...
        <Box>
          <Stack className="mainCaintainer">
            {questioncommentorderdata?.map((item: any) => {
              const commentButtonField = totalcommentbutton?.find(
                (x: any) => x.Question?.id === item?.Question?.id
              );
              questiondatarow =
                formDetails?.data?.FormInvitation[0]?.Form?.FormFields.filter(
                  (c: any) =>
                  ...
                  <>
                    ...
                    <Box className={IsDisabled ? "disabledQuestion" : ""}>
                      ...
                      <Group ml={IsDisabled ? 0 : 25} mb={15}>
                        <AddCommentButton formField={commentButtonField || item} />
                        {IsDisabled ? (
